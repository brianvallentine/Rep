...
{
    public class VG0010B_TMOVIMENTO : QueryBasis<VG0010B_TMOVIMENTO>
    {
        ...
        public VG0010B_TMOVIMENTO() { IsCursor = true; }

        public VG0010B_TMOVIMENTO(bool justACursor)
        {
        ...
        public override VG0010B_TMOVIMENTO OpenData(List<KeyValuePair<string, object>> result)
        {
            var dta = new VG0010B_TMOVIMENTO();
            var i = 0;
            ...
            dta.MDATA_RENOVACAO = result[i++].Value?.ToString();
            dta.WDATA_RENOVACAO = string.IsNullOrWhiteSpace(dta.MDATA_RENOVACAO) ? "-1" : "0";
            dta.MCOD_EMPRESA = result[i++].Value?.ToString();
            dta.WCOD_EMPRESA = string.IsNullOrWhiteSpace(dta.MCOD_EMPRESA) ? "-1" : "0";
            dta.MLOT_EMP_SEGURADO = result[i++].Value?.ToString();
            dta.WLOT_EMP_SEGURADO = string.IsNullOrWhiteSpace(dta.MLOT_EMP_SEGURADO) ? "-1" : "0";
