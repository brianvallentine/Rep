...
namespace Sias.VidaEmGrupo.DB2.VG0120B
{
    public class M_0000_000_PRINCIPAL_DB_SELECT_3_Query1 : QueryBasis<M_0000_000_PRINCIPAL_DB_SELECT_3_Query1>
    {
        ...
        {
            #region SQL_SOURCE
            /*EXEC SQL SELECT RAMO_VGAPC,
            RAMO_VG ,
            RAMO_AP ,
            NUM_RAMO_PRSTMISTA
            INTO :PARAM-RAMO-VGAP,
            :PARAM-RAMO-VG ,
            :PARAM-RAMO-AP ,
            :PARAM-RAMO-PRSTMISTA
        ...
        public string PARAM_RAMO_VGAP { get; set; }
        public string PARAM_RAMO_VG { get; set; }
        public string PARAM_RAMO_AP { get; set; }
        public string PARAM_RAMO_PRSTMISTA { get; set; }
        ...
        public override M_0000_000_PRINCIPAL_DB_SELECT_3_Query1 OpenData(List<KeyValuePair<string, object>> result)
        {
            ...
            var i = 0;
            dta.PARAM_RAMO_VGAP = result[i++].Value?.ToString();
            dta.PARAM_RAMO_VG = result[i++].Value?.ToString();
            dta.PARAM_RAMO_AP = result[i++].Value?.ToString();
            dta.PARAM_RAMO_PRSTMISTA = result[i++].Value?.ToString();
            return dta;
        }
