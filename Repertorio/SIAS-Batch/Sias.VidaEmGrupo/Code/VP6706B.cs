...
using Dclgens;
using Copies;
using Sias.VidaEmGrupo.DB2.VP6706B;

namespace Code
{
    public class VP6706B
    {
        ...
        /*"      *                  * APGCS *         A PARTIR DE 07/06/2004.     *      */
        /*"      ******************************************************************      */
        /*"      *                                 MANOEL MESSIAS      01.06.04   *      */
        ...
        /*"01           AREA-DE-WORK.*/
        public VP6706B_AREA_DE_WORK AREA_DE_WORK { get; set; } = new VP6706B_AREA_DE_WORK();
        public class VP6706B_AREA_DE_WORK : VarBasis
        {
            ...
            private _REDEF_VP6706B_FILLER_0 _filler_0 { get; set; }
            public _REDEF_VP6706B_FILLER_0 FILLER_0
            {
                get { _filler_0 = new _REDEF_VP6706B_FILLER_0(); _.Move(WDATA_REL, _filler_0); VarBasis.RedefinePassValue(WDATA_REL, _filler_0, WDATA_REL); _filler_0.ValueChanged += () => { _.Move(_filler_0, WDATA_REL); }; return _filler_0; }
                set { VarBasis.RedefinePassValue(value, _filler_0, WDATA_REL); }
            }  //Redefines
            public class _REDEF_VP6706B_FILLER_0 : VarBasis
            {
                ...
                public _REDEF_VP6706B_FILLER_0()
                {
            ...
            public VP6706B_WDAT_REL_LIT WDAT_REL_LIT { get; set; } = new VP6706B_WDAT_REL_LIT();
            public class VP6706B_WDAT_REL_LIT : VarBasis
            {
            ...
            public VP6706B_W01DTSQL W01DTSQL { get; set; } = new VP6706B_W01DTSQL();
            public class VP6706B_W01DTSQL : VarBasis
            {
            ...
            public VP6706B_WTIME_DAY WTIME_DAY { get; set; } = new VP6706B_WTIME_DAY();
            public class VP6706B_WTIME_DAY : VarBasis
            {
                /*"      10     WHH               PIC  9(002).*/
            ...
            public VP6706B_WHORA WHORA { get; set; } = new VP6706B_WHORA();
            public class VP6706B_WHORA : VarBasis
            {
                /*"      10     WHH               PIC  9(002).*/
            ...
            public VP6706B_WABEND WABEND { get; set; } = new VP6706B_WABEND();
            public class VP6706B_WABEND : VarBasis
            {
                /*"      10    FILLER              PIC  X(010) VALUE           ' VP6706B'.*/
                public StringBasis FILLER_7 { get; set; } = new StringBasis(new PIC("X", "10", "X(010)"), @" VP6706B");
        ...
        public VP6706B_V0MOVIMENTO V0MOVIMENTO { get; set; } = new VP6706B_V0MOVIMENTO();
        ...
        {
            /*" -149- MOVE '001' TO WNR-EXEC-SQL. */
            _.Move("001", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -150- EXEC SQL WHENEVER SQLWARNING CONTINUE END-EXEC. */

            /*" -153- EXEC SQL WHENEVER SQLERROR GO TO 9999-ERRO END-EXEC. */

            /*" -156- EXEC SQL WHENEVER NOT FOUND CONTINUE END-EXEC. */

            /*" -168- MOVE '9999-12-31' TO SQL-DATSEL. */
            _.Move("9999-12-31", SQL_DATSEL);

            /*" -170- MOVE '005' TO WNR-EXEC-SQL. */
            _.Move("005", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -175- PERFORM M_0000_INICIO_DB_SELECT_1 */

            M_0000_INICIO_DB_SELECT_1();

            /*" -178- IF SQLCODE EQUAL 100 */

            if (DB.SQLCODE == 100)
            {

                /*" -179- DISPLAY '*** VP6706B *** SISTEMA PREF VIDA NAO CADASTRADO' */
                _.Display($"*** VP6706B *** SISTEMA PREF VIDA NAO CADASTRADO");
            ...
            /*" -188- PERFORM M_0000_INICIO_DB_SELECT_2 */
            ...
            if (DB.SQLCODE == 100)
            {
                ...
                /*" -192- DISPLAY '*** VP6706B *** SEM PERCENTUAL DE IOF - RAMOIND ' */
                _.Display($"*** VP6706B *** SEM PERCENTUAL DE IOF - RAMOIND ");
            ...
            /*" -247- DISPLAY '*** VP6706B *** PROCESSANDO ...' . */
            _.Display($"*** VP6706B *** PROCESSANDO ...");

            /*" -250- PERFORM M-1000-PROCESSA-MOVIMENTO THRU 1000-FIM UNTIL WFIM-V0MOVIMENTO EQUAL 'S' . */
            ...
            /*" -251- DISPLAY '*** VP6706B *** MOV LIDOS       ' AC-L-V0MOVIMENTO. */
            _.Display($"*** VP6706B *** MOV LIDOS       {AREA_DE_WORK.AC_L_V0MOVIMENTO}");

            /*" -251- DISPLAY '*** VP6706B *** COMIS INSERIDAS ' AC-I-V0COMISSAO. */
            _.Display($"*** VP6706B *** COMIS INSERIDAS {AREA_DE_WORK.AC_I_V0COMISSAO}");
        ...
        {
            /*" -175- EXEC SQL SELECT DTMOVABE INTO :SQL-DTMOVABE FROM SEGUROS.V1SISTEMA WHERE IDSISTEM = 'VP' END-EXEC. */
        ...
        {
            /*" -257- MOVE '090' TO WNR-EXEC-SQL. */
            _.Move("090", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -257- PERFORM M_0002_FINALIZA_DB_CLOSE_1 */

            M_0002_FINALIZA_DB_CLOSE_1();

            /*" -261- MOVE '091' TO WNR-EXEC-SQL. */
            _.Move("091", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -261- EXEC SQL COMMIT WORK END-EXEC. */

            DatabaseConnection.Instance.CommitTransaction();

            /*" -265- DISPLAY '*** VP6706B *** TERMINO NORMAL' . */
            _.Display($"*** VP6706B *** TERMINO NORMAL");

            /*" -267- MOVE ZEROS TO RETURN-CODE. */
        ...
        {
            /*" -257- EXEC SQL CLOSE V0MOVIMENTO END-EXEC. */
        ...
        {
            /*" -229- EXEC SQL DECLARE V0MOVIMENTO CURSOR FOR SELECT A.NUM_APOLICE, A.COD_SUBGRUPO, A.COD_FONTE, A.NUM_PROPOSTA, A.TIPO_SEGURADO, A.NUM_CERTIFICADO, A.DAC_CERTIFICADO, A.COD_CLIENTE, A.COD_AGENCIADOR, A.NUM_MATRICULA, B.PREMIO_VG, B.PREMIO_AP, B.COD_OPERACAO, A.FAIXA, B.PERC_COMISSAO, B.NUM_PARCELA, B.DATA_MOVIMENTO, B.SITUACAO FROM SEGUROS.V0MOVIMENTO A, SEGUROS.COMISSOES_VP B WHERE B.SITUACAO = '0' AND B.FONTE = A.COD_FONTE AND B.NUM_PROPOSTA = A.NUM_PROPOSTA AND A.NUM_APOLICE = 109300000680 ORDER BY 1,2 END-EXEC. */
            V0MOVIMENTO = new VP6706B_V0MOVIMENTO(false);
            string GetQuery_V0MOVIMENTO()
            {
                var query = @$"SELECT A.NUM_APOLICE
							...
							AND A.NUM_APOLICE = 109300000680 
							ORDER BY 1
							,2";
        ...
        {
            /*" -234- EXEC SQL OPEN V0MOVIMENTO END-EXEC. */
        ...
        {
            /*" -188- EXEC SQL SELECT PCIOF INTO :SQL-PCIOF FROM SEGUROS.V1RAMOIND WHERE RAMO = 93 AND DTINIVIG <= :SQL-DTMOVABE AND DTTERVIG >= :SQL-DTMOVABE END-EXEC. */
        ...
        {
            /*" -275- PERFORM 1200-LE-PLANO-AGENC THRU 1200-FIM. */

            M_1200_LE_PLANO_AGENC(true);
            /*Método Suprimido por falta de linha ou apenas EXIT nome: M_1200_FIM*/


            /*" -278- PERFORM 1300-PROC-COMIS-SUBGR THRU 1300-FIM UNTIL SQL-NUM-APOL NOT EQUAL WS-APOL-ANT OR SQL-COD-SUB NOT EQUAL WS-SUBGR-ANT OR WFIM-V0MOVIMENTO EQUAL 'S' . */
        ...
        {
            /*" -288- MOVE '110' TO WNR-EXEC-SQL. */
            _.Move("110", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -308- PERFORM M_1100_FETCH_DB_FETCH_1 */

            M_1100_FETCH_DB_FETCH_1();

            /*" -311- IF SQLCODE EQUAL 100 */

            if (DB.SQLCODE == 100)
            {

                /*" -312- MOVE 'S' TO WFIM-V0MOVIMENTO */
        ...
        {
            /*" -308- EXEC SQL FETCH V0MOVIMENTO INTO :SQL-NUM-APOL, :SQL-COD-SUB, :SQL-COD-FONTE, :SQL-PROPOSTA, :SQL-TIPO-SEG, :SQL-NUM-CERT, :SQL-DAC-CERT, :SQL-COD-CLIE, :SQL-COD-AGEN, :SQL-MATRICULA, :SQL-PRM-VG, :SQL-PRM-AP, :SQL-COD-OPERAC, :SQL-FAIXA, :SQL-PERC-COMIS, :SQL-NRPARCEL, :SQL-DTMOVTO, :SQL-SITUACAO END-EXEC. */
        ...
        {
            /*" -322- MOVE SQL-NUM-APOL TO WS-APOL-ANT. */
            _.Move(SQL_NUM_APOL, AREA_DE_WORK.WS_APOL_ANT);

            /*" -322- MOVE SQL-COD-SUB TO WS-SUBGR-ANT. */
        ...
        {
            /*" -332- MOVE 'A17' TO WNR-EXEC-SQL. */
            _.Move("A17", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -337- PERFORM M_1300_PROC_COMIS_SUBGR_DB_SELECT_1 */

            M_1300_PROC_COMIS_SUBGR_DB_SELECT_1();

            /*" -340- IF SQLCODE NOT EQUAL ZEROS */

            if (DB.SQLCODE != 00)
            {

                /*" -342- DISPLAY 'PROBLEMAS NO ACESSO AO SEGURADO - V0PROPOSTAVA ' SQL-NUM-CERT ' ' SQLCODE */
            ...
            /*" -349- IF SQL-PRM-VG > ZEROES */

            if (SQL_PRM_VG > 00)
            {

                /*" -351- PERFORM 1310-PROC-COMIS-VG THRU 1310-FIM. */
            ...
            /*" -352- IF SQL-PRM-AP > ZEROES */

            if (SQL_PRM_AP > 00)
            {

                /*" -354- PERFORM 1320-PROC-COMIS-AP THRU 1320-FIM. */
        ...
        {
            /*" -337- EXEC SQL SELECT VALUE(DATA_ADMISSAO, DATE( '1900-01-01' )) INTO :SQL-DTADMIS FROM SEGUROS.V0PROPOSTAVA WHERE NRCERTIF = :SQL-NUM-CERT END-EXEC. */
        ...
        {
            /*" -361- MOVE '017' TO WNR-EXEC-SQL. */
            _.Move("017", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -372- PERFORM M_1300_UPDATE_DB_UPDATE_1 */
        ...
        {
            /*" -372- EXEC SQL UPDATE SEGUROS.COMISSOES_VP SET SITUACAO = :SQL-SITUACAO, DATA_COMISSAO = :SQL-DTMOVABE, COD_USUARIO = 'VP6706B' , TIMESTAMP = CURRENT TIMESTAMP WHERE NUM_CERTIFICADO = :SQL-NUM-CERT AND NUM_PARCELA = :SQL-NRPARCEL AND COD_OPERACAO = :SQL-COD-OPERAC AND DATA_MOVIMENTO = :SQL-DTMOVTO AND SITUACAO = '0' END-EXEC. */
        ...
        {
            /*" -377- PERFORM 1100-FETCH THRU 1100-FIM. */
        ...
        {
            /*" -387- MOVE 93 TO SQL-RAMOFR. */
            _.Move(93, SQL_RAMOFR);

            /*" -388- COMPUTE SQL-PRM-VG ROUNDED = SQL-PRM-VG / WS-PCIOF. */
            SQL_PRM_VG.Value = SQL_PRM_VG / AREA_DE_WORK.WS_PCIOF;

            /*" -390- MOVE SQL-PRM-VG TO SQL-VALBAS. */
            _.Move(SQL_PRM_VG, SQL_VALBAS);

            /*" -390- PERFORM 1350-CALCULA-COMISSAO THRU 1350-FIM. */
        ...
        {
            /*" -399- MOVE 82 TO SQL-RAMOFR. */
            _.Move(82, SQL_RAMOFR);

            /*" -400- COMPUTE SQL-PRM-AP ROUNDED = SQL-PRM-AP / WS-PCIOF. */
            SQL_PRM_AP.Value = SQL_PRM_AP / AREA_DE_WORK.WS_PCIOF;

            /*" -402- MOVE SQL-PRM-AP TO SQL-VALBAS. */
            _.Move(SQL_PRM_AP, SQL_VALBAS);

            /*" -402- PERFORM 1350-CALCULA-COMISSAO THRU 1350-FIM. */
        ...
        {
            /*" -431- MOVE '1' TO SQL-SITUACAO. */
            _.Move("1", SQL_SITUACAO);

            /*" -431- MOVE SQL-PERC-COMIS TO SQL-PCT-AGENC. */
        ...
        {
            /*" -438- COMPUTE SQL-VLCOMIS ROUNDED = (SQL-VALBAS * SQL-PCT-AGENC) / 100. */
            SQL_VLCOMIS.Value = (SQL_VALBAS * SQL_PCT_AGENC) / 100f;

            /*" -438- PERFORM 1400-GRAVA-COMIS THRU 1400-FIM. */
        ...
        private void M_1400_LOOP_GRAVA(bool isPerform = false)
        {
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                /*" -498- IF SQLCODE EQUAL -803 */

                if (DB.SQLCODE == -803)
                {

                    /*" -499- GO TO 1400-LOOP-GRAVA */
            ...
            /*" -504- ADD +1 TO AC-I-V0COMISSAO. */
            AREA_DE_WORK.AC_I_V0COMISSAO.Value = AREA_DE_WORK.AC_I_V0COMISSAO + +1;

            /*" -505- EXEC SQL WHENEVER SQLERROR GO TO 9999-ERRO END-EXEC. */
        ...
        private void M_9000_ENCERRA_SEM_MOVTO(bool isPerform = false)
        {
            ...
            /*" -523- DISPLAY '*   VP6706B - ATUALIZACAO DB COMISSOES     *' */
            _.Display($"*   VP6706B - ATUALIZACAO DB COMISSOES     *");
            ...
            /*" -526- DISPLAY '*   NAO HOUVE MOVIMENTACAO                 *' */
            _.Display($"*   NAO HOUVE MOVIMENTACAO                 *");

            /*" -528- DISPLAY '*   NESTA DATA ' WDAT-REL-LIT '                    *' */
        ...
        {
            /*" -541- MOVE SQLCODE TO WSQLCODE. */
            _.Move(DB.SQLCODE, AREA_DE_WORK.WABEND.WSQLCODE);

            /*" -543- DISPLAY WABEND. */
            _.Display(AREA_DE_WORK.WABEND);

            /*" -545- DISPLAY 'NUM-CERT ' SQL-NUM-CERT. */
            _.Display($"NUM-CERT {SQL_NUM_CERT}");

            /*" -545- EXEC SQL WHENEVER SQLERROR CONTINUE END-EXEC. */

            /*" -547- EXEC SQL ROLLBACK WORK END-EXEC. */
