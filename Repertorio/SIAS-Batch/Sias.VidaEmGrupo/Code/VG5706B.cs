...
using Dclgens;
using Copies;
using Sias.VidaEmGrupo.DB2.VG5706B;

namespace Code
{
    public class VG5706B
    {
        ...
        /*"      *   VG/AP NA MODALIDADE DE PAGAMENTO DE PARCELAS.                *      */
        /*"      *                                                                *      */
        /*"      **  APOLICES NO DECLARE CURSOR E UPDATE. <-------------------    *      */
        /*"      *                                                                *      */
        /*"      ******************************************************************      */
        /*"      *                                 FREDERICO J FONSECA 26.11.01   *      */
        /*"      *                                 * GERADO A PARTIR DO VP5706B   *      */
        /*"      ******************************************************************      */
        /*"      ******************************************************************      */
        /*"      ******************************************************************      */
        #endregion
        ...
        /*"01           AREA-DE-WORK.*/
        public VG5706B_AREA_DE_WORK AREA_DE_WORK { get; set; } = new VG5706B_AREA_DE_WORK();
        public class VG5706B_AREA_DE_WORK : VarBasis
        {
            ...
            private _REDEF_VG5706B_FILLER_0 _filler_0 { get; set; }
            public _REDEF_VG5706B_FILLER_0 FILLER_0
            {
                get { _filler_0 = new _REDEF_VG5706B_FILLER_0(); _.Move(WDATA_REL, _filler_0); VarBasis.RedefinePassValue(WDATA_REL, _filler_0, WDATA_REL); _filler_0.ValueChanged += () => { _.Move(_filler_0, WDATA_REL); }; return _filler_0; }
                set { VarBasis.RedefinePassValue(value, _filler_0, WDATA_REL); }
            }  //Redefines
            public class _REDEF_VG5706B_FILLER_0 : VarBasis
            {
                ...
                public _REDEF_VG5706B_FILLER_0()
                {
            ...
            public VG5706B_WDAT_REL_LIT WDAT_REL_LIT { get; set; } = new VG5706B_WDAT_REL_LIT();
            public class VG5706B_WDAT_REL_LIT : VarBasis
            {
            ...
            public VG5706B_W01DTSQL W01DTSQL { get; set; } = new VG5706B_W01DTSQL();
            public class VG5706B_W01DTSQL : VarBasis
            {
            ...
            public VG5706B_WTIME_DAY WTIME_DAY { get; set; } = new VG5706B_WTIME_DAY();
            public class VG5706B_WTIME_DAY : VarBasis
            {
                /*"      10     WHH               PIC  9(002).*/
            ...
            public VG5706B_WHORA WHORA { get; set; } = new VG5706B_WHORA();
            public class VG5706B_WHORA : VarBasis
            {
                /*"      10     WHH               PIC  9(002).*/
            ...
            public VG5706B_WABEND WABEND { get; set; } = new VG5706B_WABEND();
            public class VG5706B_WABEND : VarBasis
            {
                /*"      10    FILLER              PIC  X(010) VALUE           ' VG5706B'.*/
                public StringBasis FILLER_7 { get; set; } = new StringBasis(new PIC("X", "10", "X(010)"), @" VG5706B");
        ...
        public VG5706B_V0MOVIMENTO V0MOVIMENTO { get; set; } = new VG5706B_V0MOVIMENTO();
        ...
        {
            /*" -147- MOVE '001' TO WNR-EXEC-SQL. */
            _.Move("001", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -148- EXEC SQL WHENEVER SQLWARNING CONTINUE END-EXEC. */

            /*" -151- EXEC SQL WHENEVER SQLERROR GO TO 9999-ERRO END-EXEC. */

            /*" -154- EXEC SQL WHENEVER NOT FOUND CONTINUE END-EXEC. */

            /*" -166- MOVE '9999-12-31' TO SQL-DATSEL. */
            _.Move("9999-12-31", SQL_DATSEL);

            /*" -168- MOVE '005' TO WNR-EXEC-SQL. */
            _.Move("005", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -173- PERFORM M_0000_INICIO_DB_SELECT_1 */

            M_0000_INICIO_DB_SELECT_1();

            /*" -176- IF SQLCODE EQUAL 100 */

            if (DB.SQLCODE == 100)
            {

                /*" -177- DISPLAY '*** VG5706B *** SISTEMA VG NAO CADASTRADO' */
                _.Display($"*** VG5706B *** SISTEMA VG NAO CADASTRADO");
            ...
            /*" -186- PERFORM M_0000_INICIO_DB_SELECT_2 */

            M_0000_INICIO_DB_SELECT_2();

            /*" -189- IF SQLCODE EQUAL 100 */

            if (DB.SQLCODE == 100)
            {

                /*" -190- DISPLAY '*** VG5706B *** SEM PERCENTUAL DE IOF - RAMOIND ' */
                _.Display($"*** VG5706B *** SEM PERCENTUAL DE IOF - RAMOIND ");
            ...
            /*" -198- COMPUTE WS-PCIOF = 1 + (SQL-PCIOF / 100). */
            ...
            /*" -232- DISPLAY '*** VG5706B *** ABRINDO CURSOR ...' . */
            _.Display($"*** VG5706B *** ABRINDO CURSOR ...");
            ...
            /*" -245- DISPLAY '*** VG5706B *** PROCESSANDO ...' . */
            _.Display($"*** VG5706B *** PROCESSANDO ...");
            ...
            /*" -249- DISPLAY '*** VG5706B *** MOV LIDOS       ' AC-L-V0MOVIMENTO. */
            _.Display($"*** VG5706B *** MOV LIDOS       {AREA_DE_WORK.AC_L_V0MOVIMENTO}");

            /*" -249- DISPLAY '*** VG5706B *** COMIS INSERIDAS ' AC-I-V0COMISSAO. */
            _.Display($"*** VG5706B *** COMIS INSERIDAS {AREA_DE_WORK.AC_I_V0COMISSAO}");
        ...
        {
            /*" -173- EXEC SQL SELECT DTMOVABE INTO :SQL-DTMOVABE FROM SEGUROS.V1SISTEMA WHERE IDSISTEM = 'VG' END-EXEC. */
        ...
        {
            /*" -255- MOVE '090' TO WNR-EXEC-SQL. */
            _.Move("090", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -255- PERFORM M_0002_FINALIZA_DB_CLOSE_1 */

            M_0002_FINALIZA_DB_CLOSE_1();

            /*" -259- MOVE '091' TO WNR-EXEC-SQL. */
            _.Move("091", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -259- EXEC SQL COMMIT WORK END-EXEC. */

            DatabaseConnection.Instance.CommitTransaction();

            /*" -263- DISPLAY '*** VG5706B *** TERMINO NORMAL' . */
            _.Display($"*** VG5706B *** TERMINO NORMAL");
        ...
        {
            /*" -255- EXEC SQL CLOSE V0MOVIMENTO END-EXEC. */
        ...
        {
            /*" -227- EXEC SQL DECLARE V0MOVIMENTO CURSOR FOR SELECT A.NUM_APOLICE, A.COD_SUBGRUPO, A.COD_FONTE, A.NUM_PROPOSTA, A.TIPO_SEGURADO, A.NUM_CERTIFICADO, A.DAC_CERTIFICADO, A.COD_CLIENTE, A.COD_AGENCIADOR, A.NUM_MATRICULA, B.PREMIO_VG, B.PREMIO_AP, B.COD_OPERACAO, A.FAIXA, B.PERC_COMISSAO, B.NUM_PARCELA, B.DATA_MOVIMENTO, B.SITUACAO FROM SEGUROS.V0MOVIMENTO A, SEGUROS.COMISSOES_VP B WHERE B.SITUACAO = '0' AND B.FONTE = A.COD_FONTE AND B.NUM_PROPOSTA = A.NUM_PROPOSTA AND A.NUM_APOLICE = 109700000033 ORDER BY 1,2 END-EXEC. */
            V0MOVIMENTO = new VG5706B_V0MOVIMENTO(false);
            string GetQuery_V0MOVIMENTO()
            {
                var query = @$"SELECT A.NUM_APOLICE
							...
							AND A.NUM_APOLICE = 109700000033 
							ORDER BY 1
							,2";
        ...
        {
            /*" -186- EXEC SQL SELECT PCIOF INTO :SQL-PCIOF FROM SEGUROS.V1RAMOIND WHERE RAMO = 93 AND DTINIVIG <= :SQL-DTMOVABE AND DTTERVIG >= :SQL-DTMOVABE END-EXEC. */
        ...
        {
            /*" -273- PERFORM 1200-LE-PLANO-AGENC THRU 1200-FIM. */

            M_1200_LE_PLANO_AGENC(true);
            /*Método Suprimido por falta de linha ou apenas EXIT nome: M_1200_FIM*/


            /*" -276- PERFORM 1300-PROC-COMIS-SUBGR THRU 1300-FIM UNTIL SQL-NUM-APOL NOT EQUAL WS-APOL-ANT OR SQL-COD-SUB NOT EQUAL WS-SUBGR-ANT OR WFIM-V0MOVIMENTO EQUAL 'S' . */
        ...
        {
            /*" -286- MOVE '110' TO WNR-EXEC-SQL. */
            _.Move("110", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -306- PERFORM M_1100_FETCH_DB_FETCH_1 */
            ...
            if (DB.SQLCODE == 100)
            {
                ...
                /*" -310- MOVE 'S' TO WFIM-V0MOVIMENTO */
        ...
        {
            /*" -306- EXEC SQL FETCH V0MOVIMENTO INTO :SQL-NUM-APOL, :SQL-COD-SUB, :SQL-COD-FONTE, :SQL-PROPOSTA, :SQL-TIPO-SEG, :SQL-NUM-CERT, :SQL-DAC-CERT, :SQL-COD-CLIE, :SQL-COD-AGEN, :SQL-MATRICULA, :SQL-PRM-VG, :SQL-PRM-AP, :SQL-COD-OPERAC, :SQL-FAIXA, :SQL-PERC-COMIS, :SQL-NRPARCEL, :SQL-DTMOVTO, :SQL-SITUACAO END-EXEC. */
        ...
        {
            /*" -320- MOVE SQL-NUM-APOL TO WS-APOL-ANT. */
            _.Move(SQL_NUM_APOL, AREA_DE_WORK.WS_APOL_ANT);

            /*" -320- MOVE SQL-COD-SUB TO WS-SUBGR-ANT. */
        ...
        {
            /*" -329- IF SQL-PRM-VG > ZEROES */

            if (SQL_PRM_VG > 00)
            {

                /*" -331- PERFORM 1310-PROC-COMIS-VG THRU 1310-FIM. */
            ...
            /*" -332- IF SQL-PRM-AP > ZEROES */

            if (SQL_PRM_AP > 00)
            {

                /*" -334- PERFORM 1320-PROC-COMIS-AP THRU 1320-FIM. */
        ...
        {
            /*" -341- MOVE '017' TO WNR-EXEC-SQL. */
            _.Move("017", AREA_DE_WORK.WABEND.WNR_EXEC_SQL);

            /*" -352- PERFORM M_1300_UPDATE_DB_UPDATE_1 */
        ...
        {
            /*" -352- EXEC SQL UPDATE SEGUROS.COMISSOES_VP SET SITUACAO = :SQL-SITUACAO, DATA_COMISSAO = :SQL-DTMOVABE, COD_USUARIO = 'VG5706B' , TIMESTAMP = CURRENT TIMESTAMP WHERE NUM_CERTIFICADO = :SQL-NUM-CERT AND NUM_PARCELA = :SQL-NRPARCEL AND COD_OPERACAO = :SQL-COD-OPERAC AND DATA_MOVIMENTO = :SQL-DTMOVTO AND SITUACAO = '0' END-EXEC. */
        ...
        {
            /*" -357- PERFORM 1100-FETCH THRU 1100-FIM. */
        ...
        {
            /*" -367- MOVE 93 TO SQL-RAMOFR. */
            _.Move(93, SQL_RAMOFR);

            /*" -368- COMPUTE SQL-PRM-VG ROUNDED = SQL-PRM-VG / WS-PCIOF. */
            SQL_PRM_VG.Value = SQL_PRM_VG / AREA_DE_WORK.WS_PCIOF;

            /*" -370- MOVE SQL-PRM-VG TO SQL-VALBAS. */
            _.Move(SQL_PRM_VG, SQL_VALBAS);

            /*" -370- PERFORM 1350-CALCULA-COMISSAO THRU 1350-FIM. */
        ...
        {
            /*" -379- MOVE 81 TO SQL-RAMOFR. */
            _.Move(81, SQL_RAMOFR);

            /*" -380- COMPUTE SQL-PRM-AP ROUNDED = SQL-PRM-AP / WS-PCIOF. */
            SQL_PRM_AP.Value = SQL_PRM_AP / AREA_DE_WORK.WS_PCIOF;

            /*" -382- MOVE SQL-PRM-AP TO SQL-VALBAS. */
            _.Move(SQL_PRM_AP, SQL_VALBAS);

            /*" -382- PERFORM 1350-CALCULA-COMISSAO THRU 1350-FIM. */
        ...
        {
            /*" -398- PERFORM M_1350_CALCULA_COMISSAO_DB_SELECT_1 */

            M_1350_CALCULA_COMISSAO_DB_SELECT_1();

            /*" -401- IF SQL-PAGAS NOT LESS SQL-NRPARCEL */

            if (SQL_PAGAS >= SQL_NRPARCEL)
            {

                /*" -402- MOVE '1' TO SQL-SITUACAO */
            ...
            else
            {
                ...
                /*" -404- MOVE '0' TO SQL-SITUACAO */
                _.Move("0", SQL_SITUACAO);

                /*" -406- GO TO 1350-FIM. */
                /*Método Suprimido por falta de linha ou apenas EXIT nome: M_1350_FIM*/ //GOTO
                return;
            }


            /*" -406- MOVE SQL-PERC-COMIS TO SQL-PCT-AGENC. */
            _.Move(SQL_PERC_COMIS, SQL_PCT_AGENC);

        }

        [StopWatch]
        /*" M-1350-CALCULA-COMISSAO-DB-SELECT-1 */
        public void M_1350_CALCULA_COMISSAO_DB_SELECT_1()
        {
            /*" -398- EXEC SQL SELECT VALUE(COUNT(*),0) INTO :SQL-PAGAS FROM SEGUROS.V0PARCELVA WHERE NRCERTIF = :SQL-NUM-CERT AND SITUACAO = '1' END-EXEC. */

            var m_1350_CALCULA_COMISSAO_DB_SELECT_1_Query1 = new M_1350_CALCULA_COMISSAO_DB_SELECT_1_Query1()
            {
                SQL_NUM_CERT = SQL_NUM_CERT.ToString(),
            };

            var executed_1 = M_1350_CALCULA_COMISSAO_DB_SELECT_1_Query1.Execute(m_1350_CALCULA_COMISSAO_DB_SELECT_1_Query1);
            if (executed_1 != null)
            {
                _.Move(executed_1.SQL_PAGAS, SQL_PAGAS);
            }
        ...
        {
            /*" -413- COMPUTE SQL-VLCOMIS ROUNDED = (SQL-VALBAS * SQL-PCT-AGENC) / 100. */
            SQL_VLCOMIS.Value = (SQL_VALBAS * SQL_PCT_AGENC) / 100f;

            /*" -413- PERFORM 1400-GRAVA-COMIS THRU 1400-FIM. */
        ...
        {
            /*" -422- EXEC SQL WHENEVER SQLERROR CONTINUE END-EXEC. */
        ...
        private void M_1400_LOOP_GRAVA(bool isPerform = false)
        {
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                /*" -472- IF SQLCODE EQUAL -803 */

                if (DB.SQLCODE == -803)
                {

                    /*" -473- GO TO 1400-LOOP-GRAVA */
            ...
            /*" -478- ADD +1 TO AC-I-V0COMISSAO. */
            AREA_DE_WORK.AC_I_V0COMISSAO.Value = AREA_DE_WORK.AC_I_V0COMISSAO + +1;

            /*" -479- EXEC SQL WHENEVER SQLERROR GO TO 9999-ERRO END-EXEC. */
        ...
        private void M_9000_ENCERRA_SEM_MOVTO(bool isPerform = false)
        {
            ...
            /*" -497- DISPLAY '*   VG5706B - ATUALIZACAO DB COMISSOES     *' */
            _.Display($"*   VG5706B - ATUALIZACAO DB COMISSOES     *");
            ...
            /*" -500- DISPLAY '*   NAO HOUVE MOVIMENTACAO                 *' */
            _.Display($"*   NAO HOUVE MOVIMENTACAO                 *");

            /*" -502- DISPLAY '*   NESTA DATA ' WDAT-REL-LIT '                    *' */
        ...
        {
            /*" -515- MOVE SQLCODE TO WSQLCODE. */
            _.Move(DB.SQLCODE, AREA_DE_WORK.WABEND.WSQLCODE);

            /*" -517- DISPLAY WABEND. */
            _.Display(AREA_DE_WORK.WABEND);

            /*" -519- DISPLAY 'NUM-CERT ' SQL-NUM-CERT. */
            _.Display($"NUM-CERT {SQL_NUM_CERT}");

            /*" -519- EXEC SQL WHENEVER SQLERROR CONTINUE END-EXEC. */

            /*" -521- EXEC SQL ROLLBACK WORK END-EXEC. */
