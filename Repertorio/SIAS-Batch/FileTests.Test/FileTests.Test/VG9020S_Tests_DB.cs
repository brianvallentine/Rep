...
{
    [Collection("VG9020S_Tests_DB")]
    [Trait(IA_ConverterCommons.Utils.TestTraits.TestType.Name, IA_ConverterCommons.Utils.TestTraits.TestType.Values.DatabaseTesting)]
    [Trait(IA_ConverterCommons.Utils.TestTraits.Package.Name, IA_ConverterCommons.Utils.TestTraits.Package.Values.Package03)]
    public class VG9020S_Tests_DB
    {

        [Fact]
        public static void VG9020S_Database()
        {
            var program = new VG9020S();
            AppSettings.TestSet.DB_Test.Is_DB_Test = true;

            program.SDATA_DTTERVIG.Value = "2000-10-10";
            program.SDATA_MOVIMENTO.Value = "2000-10-10";
            program.VGDATA_INIVIGENCIA.Value = "2000-10-10";
            program.VGDATA_TERVIGENCIA.Value = "2000-10-10";
            program.MDATA_REFERENCIA.Value = "2000-10-10";
            program.MDATA_MOVIMENTO.Value = "2000-10-10";
            program.V1SISTEMA_DTMOVABE.Value = "2000-10-10";
            program.HHORA_OPERACAO.Value = "11:52:51";
            program.V0CAR_DTTERVIG.Value = "2000-10-10";
            ...
            try { /*7*/ program.M_160_000_UPDATE_V0SEGURAVG_DB_UPDATE_1(); } catch (Exception ex) { _.ThreatableTestError(ex); }
            try { /*8*/ program.M_180_000_ENCERRA_V0COBERAPOL_DB_UPDATE_1(); } catch (Exception ex) { _.ThreatableTestError(ex); }
            try { /*9*/ program.M_300_000_INSERE_V0COBERAPOL_DB_INSERT_1(); } catch (Exception ex) { _.ThreatableTestError(ex); }
            try { /*10*/ program.M_310_000_SELECT_V1HISTSEGVG_DB_SELECT_1(); } catch (Exception ex) { _.ThreatableTestError(ex); }
            try { /*11*/ program.M_330_000_INSERE_V0HISTSEGVG_DB_INSERT_1(); } catch (Exception ex) { _.ThreatableTestError(ex); }
            ...
            try { /*22*/ program.M_410_010_INCLUI_CARENCIAS_DB_INSERT_1(); } catch (Exception ex) { _.ThreatableTestError(ex); }
