...
namespace FileTests.Test_DB
{
    ...
    public class VG9545B_Tests_DB
    {

        [Fact]
        public static void VG9545B_Database()
        {
            ...
            try { /*5*/ program.R6000_00_CANCELA_SEGURO_DB_SELECT_1(); } catch (Exception ex) { _.ThreatableTestError(ex); }
            try { /*6*/ program.R6000_00_CANCELA_SEGURO_DB_UPDATE_1(); } catch (Exception ex) { _.ThreatableTestError(ex); }
            ...
            try { /*10*/
                ...
                program.MOVIMVGA.DCLMOVIMENTO_VGAP.MOVIMVGA_DATA_OPERACAO.Value = "2022-02-02";
                program.DATA_REFERENCIA.Value = "2023-01-01";
                program.MOVIMVGA.DCLMOVIMENTO_VGAP.MOVIMVGA_DATA_MOVIMENTO.Value = "2021-01-01";
                program.R6000_00_CANCELA_SEGURO_DB_INSERT_1(); } catch (Exception ex) { _.ThreatableTestError(ex); }
            try { /*11*/ program.R6000_00_CANCELA_SEGURO_DB_SELECT_5(); } catch (Exception ex) { _.ThreatableTestError(ex); }
            try { /*12*/ program.R6000_00_CANCELA_SEGURO_DB_SELECT_6(); } catch (Exception ex) { _.ThreatableTestError(ex); }
