...
{
    public class R6000_CONSULTA_POR_NN_SAP_DB_SELECT_1_Query1 : QueryBasis<R6000_CONSULTA_POR_NN_SAP_DB_SELECT_1_Query1>
    {
        string GetQuery()
        {
            #region SQL_SOURCE
            ...
            var query = @$"
				SELECT NUM_PROPOSTA 
							...
							,
											DATE(DTA_VENCIMENTO + QTD_DIAS_CUSTODIA DAYS)
											FROM SEGUROS.GE_CONTROLE_EMISSAO_SIGCB A
											WHERE A.NUM_NOSSO_NUMERO_SAP ='{this.GE403_NUM_NOSSO_NUMERO_SAP}'
											AND A.SEQ_CONTROLE_SIGCB =
											(SELECT MAX(B.SEQ_CONTROLE_SIGCB)
											FROM SEGUROS.GE_CONTROLE_EMISSAO_SIGCB B
											WHERE B.NUM_NOSSO_NUMERO_SAP =
        ...
        public static R6000_CONSULTA_POR_NN_SAP_DB_SELECT_1_Query1 Execute(R6000_CONSULTA_POR_NN_SAP_DB_SELECT_1_Query1 r6000_CONSULTA_POR_NN_SAP_DB_SELECT_1_Query1)
        {
            var ths = r6000_CONSULTA_POR_NN_SAP_DB_SELECT_1_Query1;
        ...
        public override R6000_CONSULTA_POR_NN_SAP_DB_SELECT_1_Query1 OpenData(List<KeyValuePair<string, object>> result)
        {
            var dta = new R6000_CONSULTA_POR_NN_SAP_DB_SELECT_1_Query1();
            var i = 0;
            dta.GE403_NUM_PROPOSTA = result[i++].Value?.ToString();
            dta.GE403_NUM_CERTIFICADO = result[i++].Value?.ToString();
            dta.GE403_NUM_PARCELA = result[i++].Value?.ToString();
