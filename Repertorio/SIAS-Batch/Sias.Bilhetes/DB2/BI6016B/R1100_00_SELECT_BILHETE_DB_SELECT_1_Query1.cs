using System;
using IA_ConverterCommons;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using System.Threading.Tasks;
using System.Linq;
using _ = IA_ConverterCommons.Statements;
using DB = IA_ConverterCommons.DatabaseBasis;

namespace Sias.Bilhetes.DB2.BI6016B
{
    public class R1100_00_SELECT_BILHETE_DB_SELECT_1_Query1 : QueryBasis<R1100_00_SELECT_BILHETE_DB_SELECT_1_Query1>
    {
        string GetQuery()
        {
            #region SQL_SOURCE
            /*EXEC SQL
            SELECT A.COD_CLIENTE ,
            A.FONTE ,
            A.RAMO ,
            A.OCORR_ENDERECO,
            A.NUM_APOLICE,
            B.COD_PRODUTO
            INTO :BILHETE-COD-CLIENTE ,
            :BILHETE-FONTE ,
            :BILHETE-RAMO ,
            :BILHETE-OCORR-ENDERECO,
            :BILHETE-NUM-APOLICE,
            :APOLICES-COD-PRODUTO
            FROM SEGUROS.BILHETE A,
            SEGUROS.APOLICES B
            WHERE A.NUM_BILHETE = :BILHETE-NUM-BILHETE
            AND A.NUM_APOLICE = B.NUM_APOLICE
            END-EXEC.
            */
            #endregion
            var query = @$"
				SELECT A.COD_CLIENTE 
							,
											A.FONTE 
							,
											A.RAMO 
							,
											A.OCORR_ENDERECO
							,
											A.NUM_APOLICE
							,
											B.COD_PRODUTO
											FROM SEGUROS.BILHETE A
							,
											SEGUROS.APOLICES B
											WHERE A.NUM_BILHETE = '{this.BILHETE_NUM_BILHETE}'
											AND A.NUM_APOLICE = B.NUM_APOLICE";

            return query;
        }
        public string BILHETE_COD_CLIENTE { get; set; }
        public string BILHETE_FONTE { get; set; }
        public string BILHETE_RAMO { get; set; }
        public string BILHETE_OCORR_ENDERECO { get; set; }
        public string BILHETE_NUM_APOLICE { get; set; }
        public string APOLICES_COD_PRODUTO { get; set; }
        public string BILHETE_NUM_BILHETE { get; set; }

        public static R1100_00_SELECT_BILHETE_DB_SELECT_1_Query1 Execute(R1100_00_SELECT_BILHETE_DB_SELECT_1_Query1 r1100_00_SELECT_BILHETE_DB_SELECT_1_Query1)
        {
            var ths = r1100_00_SELECT_BILHETE_DB_SELECT_1_Query1;
            ths.SetQuery(ths.GetQuery());

            ths.Open();
            var isFetch = ths.Fetch();

            return isFetch ? ths : null;
        }

        public override R1100_00_SELECT_BILHETE_DB_SELECT_1_Query1 OpenData(List<KeyValuePair<string, object>> result)
        {
            var dta = new R1100_00_SELECT_BILHETE_DB_SELECT_1_Query1();
            var i = 0;
            dta.BILHETE_COD_CLIENTE = result[i++].Value?.ToString();
            dta.BILHETE_FONTE = result[i++].Value?.ToString();
            dta.BILHETE_RAMO = result[i++].Value?.ToString();
            dta.BILHETE_OCORR_ENDERECO = result[i++].Value?.ToString();
            dta.BILHETE_NUM_APOLICE = result[i++].Value?.ToString();
            dta.APOLICES_COD_PRODUTO = result[i++].Value?.ToString();
            return dta;
        }

    }
}