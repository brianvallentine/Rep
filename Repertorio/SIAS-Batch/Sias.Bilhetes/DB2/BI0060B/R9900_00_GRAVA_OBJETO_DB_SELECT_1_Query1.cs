...
namespace Sias.Bilhetes.DB2.BI0060B
{
    public class R9900_00_GRAVA_OBJETO_DB_SELECT_1_Query1 : QueryBasis<R9900_00_GRAVA_OBJETO_DB_SELECT_1_Query1>
    {
        string GetQuery()
        {
            #region SQL_SOURCE
            /*EXEC SQL
            SELECT QTD_PESO_GRAMAS
            INTO :GEOBJECT-QTD-PESO-GRAMAS
            FROM SEGUROS.GE_OBJETO_ECT
            WHERE NUM_CEP = 0 AND
            NOM_PROGRAMA = 'BI0060B' AND
            COD_FORMULARIO = :GEOBJECT-COD-FORMULARIO AND
            STA_REGISTRO = 'H'
            END-EXEC.
            */
            #endregion
            var query = @$"
				SELECT QTD_PESO_GRAMAS
											...
											WHERE NUM_CEP = 0 AND
											NOM_PROGRAMA = 'BI0060B' AND
											COD_FORMULARIO = '{this.GEOBJECT_COD_FORMULARIO}' AND
											STA_REGISTRO = 'H'";
        ...
        public override R9900_00_GRAVA_OBJETO_DB_SELECT_1_Query1 OpenData(List<KeyValuePair<string, object>> result)
        {
            ...
            var i = 0;
            dta.GEOBJECT_QTD_PESO_GRAMAS = result[i++].Value?.ToString();
            return dta;
        }
