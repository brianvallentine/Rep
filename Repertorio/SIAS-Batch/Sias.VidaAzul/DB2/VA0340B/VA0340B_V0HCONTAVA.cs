...
{
    public class VA0340B_V0HCONTAVA : QueryBasis<VA0340B_V0HCONTAVA>
    {
        ...
        public VA0340B_V0HCONTAVA() { IsCursor = true; }

        public VA0340B_V0HCONTAVA(bool justACursor)
        {
        ...
        public override VA0340B_V0HCONTAVA OpenData(List<KeyValuePair<string, object>> result)
        {
            var dta = new VA0340B_V0HCONTAVA();
            var i = 0;

            dta.NRCERTIF = result[i++].Value?.ToString();
            dta.NRPARCEL = result[i++].Value?.ToString();
            dta.OCORRHISTCTA = result[i++].Value?.ToString();
