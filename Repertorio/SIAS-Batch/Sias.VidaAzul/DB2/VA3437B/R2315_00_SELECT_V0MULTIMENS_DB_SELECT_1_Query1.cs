...
namespace Sias.VidaAzul.DB2.VA3437B
{
    public class R2315_00_SELECT_V0MULTIMENS_DB_SELECT_1_Query1 : QueryBasis<R2315_00_SELECT_V0MULTIMENS_DB_SELECT_1_Query1>
    {
        string GetQuery()
        {
            ...
            WHERE IDFORM = :COBMENVG-IDFORM
            AND NUM_APOLICE = :COBMENVG-NUM-APOLICE
            AND CODSUBES = :COBMENVG-CODSUBES
            AND COD_OPERACAO = :COBMENVG-COD-OPERACAO
            WITH UR
            END-EXEC.
            */
            #endregion
            var query = @$"
				SELECT JDE
							,
											JDL
											FROM SEGUROS.COBRANCA_MENS_VGAP
											WHERE IDFORM = '{this.COBMENVG_IDFORM}'
											AND NUM_APOLICE = '{this.COBMENVG_NUM_APOLICE}'
											AND CODSUBES = '{this.COBMENVG_CODSUBES}'
											AND COD_OPERACAO = '{this.COBMENVG_COD_OPERACAO}'
											WITH UR";
