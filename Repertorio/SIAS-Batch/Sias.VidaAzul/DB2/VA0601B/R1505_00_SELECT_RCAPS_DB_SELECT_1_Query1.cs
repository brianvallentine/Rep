...
namespace Sias.VidaAzul.DB2.VA0601B
{
    public class R1505_00_SELECT_RCAPS_DB_SELECT_1_Query1 : QueryBasis<R1505_00_SELECT_RCAPS_DB_SELECT_1_Query1>
    {
        string GetQuery()
        {
            ...
            FROM SEGUROS.RCAPS
            WHERE (NUM_TITULO =:DCLRCAPS.RCAPS-NUM-TITULO OR
            NUM_CERTIFICADO =:DCLRCAPS.RCAPS-NUM-CERTIFICADO)
            END-EXEC
            */
            #endregion
            var query = @$"
				SELECT COD_FONTE
							...
							,
											VAL_RCAP
											FROM SEGUROS.RCAPS
											WHERE (NUM_TITULO ='{this.RCAPS_NUM_TITULO}' OR
											NUM_CERTIFICADO ='{this.RCAPS_NUM_CERTIFICADO}')";
