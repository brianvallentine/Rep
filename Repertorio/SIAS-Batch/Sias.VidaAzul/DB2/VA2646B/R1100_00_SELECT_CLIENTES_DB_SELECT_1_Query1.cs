using System;
using IA_ConverterCommons;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using System.Threading.Tasks;
using System.Linq;
using _ = IA_ConverterCommons.Statements;
using DB = IA_ConverterCommons.DatabaseBasis;

namespace Sias.VidaAzul.DB2.VA2646B
{
    public class R1100_00_SELECT_CLIENTES_DB_SELECT_1_Query1 : QueryBasis<R1100_00_SELECT_CLIENTES_DB_SELECT_1_Query1>
    {
        string GetQuery()
        {
            #region SQL_SOURCE
            /*EXEC SQL
            SELECT
            CGCCPF ,
            NOME_RAZAO ,
            TIPO_PESSOA ,
            DATA_NASCIMENTO,
            IDE_SEXO,
            ESTADO_CIVIL
            INTO
            :CLIENTES-CGCCPF ,
            :CLIENTES-NOME-RAZAO,
            :CLIENTES-TIPO-PESSOA,
            :CLIENTES-DATA-NASCIMENTO :VIND-DTNASC ,
            :CLIENTES-IDE-SEXO :VIND-IDE-SEXO,
            :CLIENTES-ESTADO-CIVIL :VIND-ESTCIV
            FROM SEGUROS.CLIENTES
            WHERE COD_CLIENTE = :PROPOVA-COD-CLIENTE
            END-EXEC.
            */
            #endregion
            var query = @$"
				SELECT
											CGCCPF 
							,
											NOME_RAZAO 
							,
											TIPO_PESSOA 
							,
											DATA_NASCIMENTO
							,
											IDE_SEXO
							,
											ESTADO_CIVIL
											FROM SEGUROS.CLIENTES
											WHERE COD_CLIENTE = '{this.PROPOVA_COD_CLIENTE}'";

            return query;
        }
        public string CLIENTES_CGCCPF { get; set; }
        public string CLIENTES_NOME_RAZAO { get; set; }
        public string CLIENTES_TIPO_PESSOA { get; set; }
        public string CLIENTES_DATA_NASCIMENTO { get; set; }
        public string VIND_DTNASC { get; set; }
        public string CLIENTES_IDE_SEXO { get; set; }
        public string VIND_IDE_SEXO { get; set; }
        public string CLIENTES_ESTADO_CIVIL { get; set; }
        public string VIND_ESTCIV { get; set; }
        public string PROPOVA_COD_CLIENTE { get; set; }

        public static R1100_00_SELECT_CLIENTES_DB_SELECT_1_Query1 Execute(R1100_00_SELECT_CLIENTES_DB_SELECT_1_Query1 r1100_00_SELECT_CLIENTES_DB_SELECT_1_Query1)
        {
            var ths = r1100_00_SELECT_CLIENTES_DB_SELECT_1_Query1;
            ths.SetQuery(ths.GetQuery());

            ths.Open();
            var isFetch = ths.Fetch();

            return isFetch ? ths : null;
        }

        public override R1100_00_SELECT_CLIENTES_DB_SELECT_1_Query1 OpenData(List<KeyValuePair<string, object>> result)
        {
            var dta = new R1100_00_SELECT_CLIENTES_DB_SELECT_1_Query1();
            var i = 0;
            dta.CLIENTES_CGCCPF = result[i++].Value?.ToString();
            dta.CLIENTES_NOME_RAZAO = result[i++].Value?.ToString();
            dta.CLIENTES_TIPO_PESSOA = result[i++].Value?.ToString();
            dta.CLIENTES_DATA_NASCIMENTO = result[i++].Value?.ToString();
            dta.VIND_DTNASC = string.IsNullOrWhiteSpace(dta.CLIENTES_DATA_NASCIMENTO) ? "-1" : "0";
            dta.CLIENTES_IDE_SEXO = result[i++].Value?.ToString();
            dta.VIND_IDE_SEXO = string.IsNullOrWhiteSpace(dta.CLIENTES_IDE_SEXO) ? "-1" : "0";
            dta.CLIENTES_ESTADO_CIVIL = result[i++].Value?.ToString();
            dta.VIND_ESTCIV = string.IsNullOrWhiteSpace(dta.CLIENTES_ESTADO_CIVIL) ? "-1" : "0";
            return dta;
        }

    }
}