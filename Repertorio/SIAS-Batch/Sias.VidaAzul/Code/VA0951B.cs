...
using Dclgens;
using Copies;
using Sias.VidaAzul.DB2.VA0951B;

namespace Code
{
    public class VA0951B
    {
        ...
        /*"      ******************************************************************      */
        /*"      *     GERA ARQUIVO COM AS EMISSOES AUTOMATICAS                   *      */
        ...
        /*"      *                                                                *      */
        /*"      *   EM 11/09/2014 - FRANK CARVALHO (INDRA COMPANY)               *      */
        /*"      *                                       PROCURE POR V.06         *      */
        /*"      *----------------------------------------------------------------*      */
        /*"      *   VERSAO 05 - CAD 84.841                                       *      */
        /*"      *             - INCLUSAO DE CONTROLE DE SELECAO DOS CERTIFICADOS *      */
        /*"      *               PARA NAO DEIXAR DE ENVIAR OS CERTIFICADOS REPRE- *      */
        /*"      *               SADOS POR FALTA DE PROCESSAMENTO (ABEND)         *      */
        /*"      *                                                                *      */
        /*"      *   EM 11/03/2014 - ELIERMES OLIVEIRA                            *      */
        /*"      *                                       PROCURE POR V.05         *      */
        /*"      *----------------------------------------------------------------*      */
        /*"      *                                                                *      */
        ...
        /*"      *                                                                *      */
        /*"      *----------------------------------------------------------------*      */
        /*"      *   VERSAO 03 - ATENDE CAD 16 677  - ACRESCENTA CLIENTES QUE NAO *      */
        /*"      *               ESTAVAM SAINDO NESTE RELATORIO.                  *      */
        /*"      *                                                                *      */
        /*"      *   EM 12/11/2008 - JUSTINO  (FAST COMPUTER) PROCURE   POR V.03  *      */
        /*"      ******************************************************************      */
        ...
        public FileBasis _AVA0951B { get; set; } = new FileBasis(new PIC("X", "500", "X(500)"));

        public FileBasis AVA0951B
        {
            get
            {
                _.Move(REG_AVA0951B, _AVA0951B); VarBasis.RedefinePassValue(REG_AVA0951B, _AVA0951B, REG_AVA0951B); return _AVA0951B;
            }
        }
        public SortBasis<VA0951B_REG_SVA0951B> SVA0951B { get; set; } = new SortBasis<VA0951B_REG_SVA0951B>(new VA0951B_REG_SVA0951B());
        /*"01            REG-AVA0951B        PIC X(500).*/
        public StringBasis REG_AVA0951B { get; set; } = new StringBasis(new PIC("X", "500", "X(500)."), @"");
        /*"01            REG-SVA0951B.*/
        public VA0951B_REG_SVA0951B REG_SVA0951B { get; set; } = new VA0951B_REG_SVA0951B();
        public class VA0951B_REG_SVA0951B : VarBasis
        {
        ...
        /*"77            V1SIST-DTMOVABE-3M  PIC  X(10).*/
        public StringBasis V1SIST_DTMOVABE_3M { get; set; } = new StringBasis(new PIC("X", "10", "X(10)."), @"");
        ...
        /*"01  WORK-AREA.*/
        public VA0951B_WORK_AREA WORK_AREA { get; set; } = new VA0951B_WORK_AREA();
        public class VA0951B_WORK_AREA : VarBasis
        {
            /*"    05        DATA-SQL.*/
            public VA0951B_DATA_SQL DATA_SQL { get; set; } = new VA0951B_DATA_SQL();
            public class VA0951B_DATA_SQL : VarBasis
            {
            ...
            private _REDEF_VA0951B_WS_DTH_CRITICA_R _ws_dth_critica_r { get; set; }
            public _REDEF_VA0951B_WS_DTH_CRITICA_R WS_DTH_CRITICA_R
            {
                get { _ws_dth_critica_r = new _REDEF_VA0951B_WS_DTH_CRITICA_R(); _.Move(WS_DTH_CRITICA, _ws_dth_critica_r); VarBasis.RedefinePassValue(WS_DTH_CRITICA, _ws_dth_critica_r, WS_DTH_CRITICA); _ws_dth_critica_r.ValueChanged += () => { _.Move(_ws_dth_critica_r, WS_DTH_CRITICA); }; return _ws_dth_critica_r; }
                set { VarBasis.RedefinePassValue(value, _ws_dth_critica_r, WS_DTH_CRITICA); }
            }  //Redefines
            public class _REDEF_VA0951B_WS_DTH_CRITICA_R : VarBasis
            {
                ...
                public _REDEF_VA0951B_WS_DTH_CRITICA_R()
                {
            ...
            private _REDEF_VA0951B_CANAL _canal { get; set; }
            public _REDEF_VA0951B_CANAL CANAL
            {
                get { _canal = new _REDEF_VA0951B_CANAL(); _.Move(W_NUM_PROPOSTA, _canal); VarBasis.RedefinePassValue(W_NUM_PROPOSTA, _canal, W_NUM_PROPOSTA); _canal.ValueChanged += () => { _.Move(_canal, W_NUM_PROPOSTA); }; return _canal; }
                set { VarBasis.RedefinePassValue(value, _canal, W_NUM_PROPOSTA); }
            }  //Redefines
            public class _REDEF_VA0951B_CANAL : VarBasis
            {
        ...
        private _REDEF_VA0951B_FILLER_3 _filler_3 { get; set; }
        public _REDEF_VA0951B_FILLER_3 FILLER_3
        {
            get { _filler_3 = new _REDEF_VA0951B_FILLER_3(); _.Move(WPAR_PARAMETROS, _filler_3); VarBasis.RedefinePassValue(WPAR_PARAMETROS, _filler_3, WPAR_PARAMETROS); _filler_3.ValueChanged += () => { _.Move(_filler_3, WPAR_PARAMETROS); }; return _filler_3; }
            set { VarBasis.RedefinePassValue(value, _filler_3, WPAR_PARAMETROS); }
        }  //Redefines
        public class _REDEF_VA0951B_FILLER_3 : VarBasis
        {
            ...
            /*"  05        WABEND.*/
            public VA0951B_WABEND WABEND { get; set; } = new VA0951B_WABEND();
            public class VA0951B_WABEND : VarBasis
            {
                /*"    10      FILLER              PIC  X(010) VALUE           ' VA0951B'.*/
            }

            public _REDEF_VA0951B_FILLER_3()
            {
        ...
        public StringBasis FILLER_4 { get; set; } = new StringBasis(new PIC("X", "10", "X(010)"), @" VA0951B");
        ...
        /*"    05        LD00.*/
        public VA0951B_LD00 LD00 { get; set; } = new VA0951B_LD00();
        public class VA0951B_LD00 : VarBasis
        {
            /*"      10      FILLER              PIC X(07)   VALUE 'VA0951B'.*/
            public StringBasis FILLER_7 { get; set; } = new StringBasis(new PIC("X", "7", "X(07)"), @"VA0951B");
            /*"      10      FILLER              PIC X(08)   VALUE  SPACES.*/
            public StringBasis FILLER_8 { get; set; } = new StringBasis(new PIC("X", "8", "X(08)"), @"");
            /*"      10      FILLER              PIC X(50)   VALUE            'PROPOSTAS ACEITAS AUTOMATICAMENTE'.*/
            public StringBasis FILLER_9 { get; set; } = new StringBasis(new PIC("X", "50", "X(50)"), @"PROPOSTAS ACEITAS AUTOMATICAMENTE");
        ...
        public VA0951B_LD00_1 LD00_1 { get; set; } = new VA0951B_LD00_1();
        public class VA0951B_LD00_1 : VarBasis
        {
            ...
            public StringBasis FILLER_44 { get; set; } = new StringBasis(new PIC("X", "7", "X(07)"), @"AGENCIA");
            /*"      10      FILLER              PIC X(01)   VALUE ';'.*/
            public StringBasis FILLER_45 { get; set; } = new StringBasis(new PIC("X", "1", "X(01)"), @";");
            /*"      10      FILLER              PIC X(06)   VALUE 'PREMIO'.*/
            public StringBasis FILLER_46 { get; set; } = new StringBasis(new PIC("X", "6", "X(06)"), @"PREMIO");
            /*"      10      FILLER              PIC X(01)   VALUE ';'.*/
            public StringBasis FILLER_47 { get; set; } = new StringBasis(new PIC("X", "1", "X(01)"), @";");
            /*"      10      FILLER              PIC X(06)   VALUE 'IMP.SG'.*/
            public StringBasis FILLER_48 { get; set; } = new StringBasis(new PIC("X", "6", "X(06)"), @"IMP.SG");
            /*"      10      FILLER              PIC X(01)   VALUE ';'.*/
            public StringBasis FILLER_49 { get; set; } = new StringBasis(new PIC("X", "1", "X(01)"), @";");
            /*"      10      FILLER              PIC X(07)   VALUE 'USUARIO'.*/
            public StringBasis FILLER_50 { get; set; } = new StringBasis(new PIC("X", "7", "X(07)"), @"USUARIO");
            /*"    05        LD01.*/
        }
        public VA0951B_LD01 LD01 { get; set; } = new VA0951B_LD01();
        public class VA0951B_LD01 : VarBasis
        {
        ...
        public VA0951B_TABELA_FONTES1 TABELA_FONTES1 { get; set; } = new VA0951B_TABELA_FONTES1();
        public class VA0951B_TABELA_FONTES1 : VarBasis
        {
            /*"      10      TAB-FTE1            OCCURS 99 TIMES.*/
            public ListBasis<VA0951B_TAB_FTE1> TAB_FTE1 { get; set; } = new ListBasis<VA0951B_TAB_FTE1>(99);
            public class VA0951B_TAB_FTE1 : VarBasis
            {
        ...
        public VA0951B_TABELA_FONTES TABELA_FONTES { get; set; } = new VA0951B_TABELA_FONTES();
        public class VA0951B_TABELA_FONTES : VarBasis
        {
            /*"      10      TAB-FTE             OCCURS 99 TIMES.*/
            public ListBasis<VA0951B_TAB_FTE> TAB_FTE { get; set; } = new ListBasis<VA0951B_TAB_FTE>(99);
            public class VA0951B_TAB_FTE : VarBasis
            {
        ...
        public VA0951B_TABELA_ESCNEG TABELA_ESCNEG { get; set; } = new VA0951B_TABELA_ESCNEG();
        public class VA0951B_TABELA_ESCNEG : VarBasis
        {
            /*"      10      TAB-EN              OCCURS 200 TIMES                                  INDEXED BY I01.*/
            public ListBasis<VA0951B_TAB_EN> TAB_EN { get; set; } = new ListBasis<VA0951B_TAB_EN>(200);
            public class VA0951B_TAB_EN : VarBasis
            {
        ...
        public VA0951B_TCOMIS TCOMIS { get; set; } = new VA0951B_TCOMIS();
        public VA0951B_CERROSP CERROSP { get; set; } = new VA0951B_CERROSP();
        public VA0951B_CFONTES CFONTES { get; set; } = new VA0951B_CFONTES();
        public VA0951B_CESCNEG CESCNEG { get; set; } = new VA0951B_CESCNEG();
        ...
        [StopWatch]
        /*" Execute */
        public dynamic Execute(StringBasis WPAR_PARAMETROS_P, string SVA0951B_FILE_NAME_P, string AVA0951B_FILE_NAME_P) //PROCEDURE DIVISION 
        /**/
        {
            try
            {
                this.WPAR_PARAMETROS.Value = WPAR_PARAMETROS_P.Value;
                SVA0951B.SetFile(SVA0951B_FILE_NAME_P);
                AVA0951B.SetFile(AVA0951B_FILE_NAME_P);
        ...
        {
            /*" -466- EXEC SQL WHENEVER SQLWARNING CONTINUE END-EXEC. */

            /*" -469- EXEC SQL WHENEVER SQLERROR GO TO R9999-00-ROT-ERRO END-EXEC. */

            /*" -472- EXEC SQL WHENEVER NOT FOUND CONTINUE END-EXEC. */

            /*" -473- PERFORM R0000-00-PRINCIPAL. */
        ...
        private void R0000_00_PRINCIPAL_SECTION()
        {
            ...
            /*" -481- DISPLAY '          PROGRAMA EM EXECUCAO VA0951B           ' */
            _.Display($"          PROGRAMA EM EXECUCAO VA0951B           ");

            /*" -482- DISPLAY '                              ' */
            _.Display($"                              ");

            /*" -483- DISPLAY 'VERSAO V.07: ' FUNCTION WHEN-COMPILED ' - 402.982' */
            ...
            /*" -492- DISPLAY 'INICIOU PROCESSAMENTO EM ' FUNCTION CURRENT-DATE(07:2) '/' FUNCTION CURRENT-DATE(05:2) '/' FUNCTION CURRENT-DATE(01:4) ' AS ' FUNCTION CURRENT-DATE(09:2) ':' FUNCTION CURRENT-DATE(11:2) ':' FUNCTION CURRENT-DATE(13:2) */
            ...
            /*" -500- MOVE '000' TO WNR-EXEC-SQL. */
            _.Move("000", WNR_EXEC_SQL);

            /*" -504- INITIALIZE TABELA-FONTES TABELA-ESCNEG LD01. */
            _.Initialize(
                TABELA_FONTES
                , TABELA_ESCNEG
                , LD01
            ...
            /*" -506- ACCEPT WPAR-PARAMETROS FROM SYSIN. */
            /*-Accept convertido para parametro de entrada...*/

            /*" -509- MOVE 'NAO' TO WS-ERRO-DATA. */
            _.Move("NAO", WORK_AREA.WS_ERRO_DATA);

            /*" -510- IF WPAR-ROTINA EQUAL 'E' */

            if (FILLER_3.WPAR_ROTINA == "E")
            {

                /*" -512- IF WPAR-INICIO EQUAL '00000000' AND WPAR-FIM EQUAL '00000000' */

                if (FILLER_3.WPAR_INICIO == "00000000" && FILLER_3.WPAR_FIM == "00000000")
                {

                    /*" -514- DISPLAY '*** NAO HOUVE SOLICITACAO  ' WPAR-PARAMETROS */
                ...
                /*" -519- MOVE WPAR-INICIO TO WS-DTH-CRITICA-R */
                _.Move(FILLER_3.WPAR_INICIO, WORK_AREA.WS_DTH_CRITICA_R);

                /*" -521- PERFORM R0400-00-CONSISTE-DATA */

                R0400_00_CONSISTE_DATA_SECTION();

                /*" -522- IF WS-ERRO-DATA EQUAL 'NAO' */

                if (WORK_AREA.WS_ERRO_DATA == "NAO")
                {

                    /*" -525- MOVE WPAR-FIM TO WS-DTH-CRITICA-R */
                    _.Move(FILLER_3.WPAR_FIM, WORK_AREA.WS_DTH_CRITICA_R);

                    /*" -526- PERFORM R0400-00-CONSISTE-DATA */
            ...
            /*" -532- IF WPAR-ROTINA EQUAL ( 'E' OR 'D' ) AND WS-ERRO-DATA EQUAL 'NAO' NEXT SENTENCE */
            ...
            else
            {
                ...
                /*" -535- DISPLAY '*** PROBLEMAS NOS PARAMETROS INFORMADOS ' WPAR-PARAMETROS */
            ...
            /*" -540- PERFORM R3100-00-DECLARE-FONTES. */

            R3100_00_DECLARE_FONTES_SECTION();

            /*" -542- PERFORM R3110-00-FETCH-FONTES. */

            R3110_00_FETCH_FONTES_SECTION();

            /*" -543- IF WFIM-FONTES NOT EQUAL SPACES */

            if (!WORK_AREA.WFIM_FONTES.IsEmpty())
            {

                /*" -544- DISPLAY '0000 - PROBLEMA NO FETCH DA FONTES      ' */
            ...
            /*" -549- PERFORM R3120-00-CARREGA-FONTES UNTIL WFIM-FONTES EQUAL 'S' . */
            ...
            /*" -551- PERFORM R3200-00-DECLARE-ESCNEG. */

            R3200_00_DECLARE_ESCNEG_SECTION();

            /*" -553- PERFORM R3210-00-FETCH-ESCNEG. */

            R3210_00_FETCH_ESCNEG_SECTION();

            /*" -554- IF WFIM-ESCNEG NOT EQUAL SPACES */

            if (!WORK_AREA.WFIM_ESCNEG.IsEmpty())
            {

                /*" -555- DISPLAY '0000 - PROBLEMA NO FETCH DA ESCNEG      ' */
            ...
            /*" -559- PERFORM R0100-00-SELECT-V1SISTEMA. */

            R0100_00_SELECT_V1SISTEMA_SECTION();

            /*" -560- IF WFIM-V1SISTEMA NOT EQUAL SPACES */

            if (!WORK_AREA.WFIM_V1SISTEMA.IsEmpty())
            {

                /*" -561- DISPLAY '*** PROBLEMAS NA V0SISTEMA' */
                _.Display($"*** PROBLEMAS NA V0SISTEMA");

                /*" -562- MOVE 9 TO RETURN-CODE */
            ...
            /*" -565- IF WPAR-ROTINA EQUAL 'E' */

            if (FILLER_3.WPAR_ROTINA == "E")
            {

                /*" -566- PERFORM R0300-00-MONTA-DATA-PARAMETROS */
            ...
            /*" -570- PERFORM R0900-00-DECLARE-PROPOVA. */

            R0900_00_DECLARE_PROPOVA_SECTION();

            /*" -572- PERFORM R0900-00-FETCH-PROPOVA. */

            R0900_00_FETCH_PROPOVA_SECTION();

            /*" -573- IF WFIM-V0COMISICOB EQUAL 'S' */

            if (WORK_AREA.WFIM_V0COMISICOB == "S")
            {

                /*" -574- PERFORM R9800-00-ENCERRA-SEM-SOLIC */

                R9800_00_ENCERRA_SEM_SOLIC_SECTION();

                /*" -575- MOVE ZEROS TO RETURN-CODE */
            ...
            /*" -580- MOVE +400000 TO SORT-FILE-SIZE. */
            _.Move(+400000, SORT_FILE_SIZE);

            /*" -588- SORT SVA0951B ON ASCENDING KEY SVA-APOLICE SVA-CODSUBES SVA-CDFONTE SVA-CDESCNEG SVA-NRCERTIF INPUT PROCEDURE R0010-00-SELECIONA THRU R0010-FIM OUTPUT PROCEDURE R0020-00-GERA-ARQ THRU R0020-FIM. */
            SORT_RETURN.Value = SVA0951B.Sort("SVA-APOLICE,SVA-CODSUBES,SVA-CDFONTE,SVA-CDESCNEG,SVA-NRCERTIF", () => R0010_00_SELECIONA_SECTION(), () => R0020_00_GERA_ARQ_SECTION());

            /*" -591- IF SORT-RETURN NOT EQUAL ZEROS */

            if (SORT_RETURN != 00)
            {

                /*" -592- DISPLAY '*** VA0951B *** PROBLEMAS NO SORT ' */
                _.Display($"*** VA0951B *** PROBLEMAS NO SORT ");

                /*" -593- DISPLAY '*** VA0951B *** SORT-RETURN = ' SORT-RETURN */
                _.Display($"*** VA0951B *** SORT-RETURN = {SORT_RETURN}");

                /*" -594- MOVE 9 TO RETURN-CODE */
                _.Move(9, RETURN_CODE);
        ...
        {
            /*" -600- MOVE ZEROS TO RETURN-CODE. */
            _.Move(0, RETURN_CODE);

            /*" -600- EXEC SQL COMMIT WORK END-EXEC. */
            ...
            /*" -604- DISPLAY 'LIDOS V0COMISICOBVA      ' AC-LIDOS. */
            _.Display($"LIDOS V0COMISICOBVA      {AC_LIDOS}");

            /*" -605- DISPLAY 'DESPREZADOS 1            ' AC-DESP-1. */
            _.Display($"DESPREZADOS 1            {AC_DESP_1}");

            /*" -606- DISPLAY 'DESPREZADOS 2            ' AC-DESP-2. */
            _.Display($"DESPREZADOS 2            {AC_DESP_2}");

            /*" -607- DISPLAY 'DESPREZADOS 3            ' AC-DESP-3. */
            _.Display($"DESPREZADOS 3            {AC_DESP_3}");

            /*" -608- DISPLAY 'DESPREZADOS 4            ' AC-DESP-4. */
            _.Display($"DESPREZADOS 4            {AC_DESP_4}");

            /*" -609- DISPLAY 'DESPREZADOS 5            ' AC-DESP-5. */
            _.Display($"DESPREZADOS 5            {AC_DESP_5}");

            /*" -610- DISPLAY 'DESPREZADOS 6            ' AC-DESP-6. */
            _.Display($"DESPREZADOS 6            {AC_DESP_6}");

            /*" -612- DISPLAY 'DESPREZADOS 7            ' AC-DESP-7. */
            _.Display($"DESPREZADOS 7            {AC_DESP_7}");

            /*" -614- DISPLAY '*** VA0951B - TERMINO NORMAL ***' */
            _.Display($"*** VA0951B - TERMINO NORMAL ***");
        ...
        {
            /*" -628- PERFORM R1000-00-PROCESSA-INPUT UNTIL WFIM-V0COMISICOB EQUAL 'S' . */
            ...
            /*" -628- EXEC SQL COMMIT WORK END-EXEC. */
        ...
        {
            /*" -641- OPEN OUTPUT AVA0951B. */
            AVA0951B.Open(REG_AVA0951B);

            /*" -642- WRITE REG-AVA0951B FROM LD00. */
            _.Move(LD00.GetMoveValues(), REG_AVA0951B);

            AVA0951B.Write(REG_AVA0951B.GetMoveValues().ToString());

            /*" -644- WRITE REG-AVA0951B FROM LD00-1. */
            _.Move(LD00_1.GetMoveValues(), REG_AVA0951B);

            AVA0951B.Write(REG_AVA0951B.GetMoveValues().ToString());

            /*" -646- PERFORM R2400-00-LE-SORT. */

            R2400_00_LE_SORT_SECTION();

            /*" -649- PERFORM R2000-00-PROCESSA-OUTPUT UNTIL WFIM-SORT EQUAL 'S' . */
        ...
        {
            /*" -662- MOVE '010' TO WNR-EXEC-SQL. */
            _.Move("010", WNR_EXEC_SQL);

            /*" -671- PERFORM R0100_00_SELECT_V1SISTEMA_DB_SELECT_1 */
            ...
            if (DB.SQLCODE == 100)
            {
                ...
                /*" -675- DISPLAY 'VA0951B - SISTEMA VA NAO ESTA CADASTRADO' */
                _.Display($"VA0951B - SISTEMA VA NAO ESTA CADASTRADO");

                /*" -676- MOVE 'S' TO WFIM-V1SISTEMA */
                _.Move("S", WORK_AREA.WFIM_V1SISTEMA);

                /*" -678- GO TO R0100-99-SAIDA. */
            ...
            /*" -685- PERFORM R0100_00_SELECT_V1SISTEMA_DB_SELECT_2 */

            R0100_00_SELECT_V1SISTEMA_DB_SELECT_2();

            /*" -688- IF SQLCODE NOT EQUAL ZEROES */

            if (DB.SQLCODE != 00)
            {

                /*" -689- DISPLAY 'VA0951B - PROBLEMAS NO ACESSO A V0CONTROCNAB' */
                _.Display($"VA0951B - PROBLEMAS NO ACESSO A V0CONTROCNAB");

                /*" -692- GO TO R9999-00-ROT-ERRO. */
            ...
            /*" -694- MOVE V0CONT-DATCEF TO DATA-SQL. */
            _.Move(V0CONT_DATCEF, WORK_AREA.DATA_SQL);

            /*" -696- MOVE DATA-SQL TO WHOST-DTFINAL */
            _.Move(WORK_AREA.DATA_SQL, WHOST_DTFINAL);

            /*" -697- MOVE 01 TO DIA-SQL */
            _.Move(01, WORK_AREA.DATA_SQL.DIA_SQL);

            /*" -698- MOVE DATA-SQL TO WHOST-DTINICIAL */
            _.Move(WORK_AREA.DATA_SQL, WHOST_DTINICIAL);

            /*" -699- MOVE V1SIST-DTMOVABE(1:4) TO LD00-DATAINI(7:4) */
            ...
            /*" -701- MOVE V1SIST-DTMOVABE(6:2) TO LD00-DATAINI(4:2) */
            ...
            /*" -704- MOVE V1SIST-DTMOVABE(9:2) TO LD00-DATAINI(1:2). */
            _.MoveAtPosition(V1SIST_DTMOVABE.Substring(9, 2), LD00.LD00_DATAINI, 1, 2);

            /*" -705- MOVE V1SIST-DTMOVABE-3M TO WHOST-DT-INICIO */
            _.Move(V1SIST_DTMOVABE_3M, WHOST_DT_INICIO);

            /*" -705- MOVE V1SIST-DTMOVABE TO WHOST-DT-FIM. */
            _.Move(V1SIST_DTMOVABE, WHOST_DT_FIM);
        ...
        {
            /*" -671- EXEC SQL SELECT DTMOVABE, DATE(DTMOVABE - 3 MONTHS), CURRENT DATE INTO :V1SIST-DTMOVABE, :V1SIST-DTMOVABE-3M, :WHOST-DT-CORRENTE FROM SEGUROS.V1SISTEMA WHERE IDSISTEM = 'VA' END-EXEC. */
            ...
            if (executed_1 != null)
            {
                ...
                _.Move(executed_1.V1SIST_DTMOVABE_3M, V1SIST_DTMOVABE_3M);
        ...
        {
            /*" -685- EXEC SQL SELECT MAX(DATCEF) INTO :V0CONT-DATCEF FROM SEGUROS.V0CONTROCNAB WHERE NRCTACED = 63000300001004 END-EXEC. */
        ...
        private void R0300_00_MONTA_DATA_PARAMETROS_SECTION()
        {
            ...
            /*" -753- MOVE WPAR-FIM (5:2) TO LD00-DATAFIN(4:2) */
            _.MoveAtPosition(FILLER_3.WPAR_FIM.Substring(5, 2), LD00.LD00_DATAFIN, 4, 2);

            /*" -753- MOVE WPAR-FIM (5:2) TO WHOST-DT-FIM(6:2) */
            _.MoveAtPosition(FILLER_3.WPAR_FIM.Substring(5, 2), WHOST_DT_FIM, 6, 2);

            /*" -757- MOVE WPAR-FIM (7:2) TO LD00-DATAFIN(1:2) */
            _.MoveAtPosition(FILLER_3.WPAR_FIM.Substring(7, 2), LD00.LD00_DATAFIN, 1, 2);

            /*" -757- MOVE WPAR-FIM (7:2) TO WHOST-DT-FIM(9:2) */
            _.MoveAtPosition(FILLER_3.WPAR_FIM.Substring(7, 2), WHOST_DT_FIM, 9, 2);

            /*" -763- MOVE '/' TO LD00-DATAFIN(6:1) */
            _.MoveAtPosition("/", LD00.LD00_DATAFIN, 6, 1);

            /*" -763- MOVE '/' TO LD00-DATAFIN(3:1) */
            _.MoveAtPosition("/", LD00.LD00_DATAFIN, 3, 1);

            /*" -763- MOVE '/' TO LD00-DATAINI(6:1) */
            _.MoveAtPosition("/", LD00.LD00_DATAINI, 6, 1);

            /*" -763- MOVE '/' TO LD00-DATAINI(3:1) */
            _.MoveAtPosition("/", LD00.LD00_DATAINI, 3, 1);

            /*" -766- MOVE '  A  ' TO LD00-A */
            _.Move("  A  ", LD00.LD00_A);

            /*" -772- MOVE '-' TO WHOST-DT-FIM(8:1). */
            _.MoveAtPosition("-", WHOST_DT_FIM, 8, 1);

            /*" -772- MOVE '-' TO WHOST-DT-FIM(5:1) */
            _.MoveAtPosition("-", WHOST_DT_FIM, 5, 1);

            /*" -772- MOVE '-' TO WHOST-DT-INICIO(8:1) */
            _.MoveAtPosition("-", WHOST_DT_INICIO, 8, 1);

            /*" -772- MOVE '-' TO WHOST-DT-INICIO(5:1) */
            _.MoveAtPosition("-", WHOST_DT_INICIO, 5, 1);

            /*" -774- PERFORM R3116-00-UPDATE-PREP-MVMTO */

            R3116_00_UPDATE_PREP_MVMTO_SECTION();

            /*" -775- DISPLAY '------------------------------------' */
            _.Display($"------------------------------------");

            /*" -776- DISPLAY '***  PARAMETROS DE PROCESSAMENTO ***' */
            _.Display($"***  PARAMETROS DE PROCESSAMENTO ***");

            /*" -777- DISPLAY '------------------------------------' */
            _.Display($"------------------------------------");

            /*" -778- DISPLAY '*** TIPO DE ROTINA   ' WPAR-ROTINA */
            _.Display($"*** TIPO DE ROTINA   {FILLER_3.WPAR_ROTINA}");

            /*" -779- DISPLAY '------------------------------------' */
            _.Display($"------------------------------------");

            /*" -780- DISPLAY '*** DATA DE INICIO   ' LD00-DATAINI */
            _.Display($"*** DATA DE INICIO   {LD00.LD00_DATAINI}");

            /*" -781- DISPLAY '*** DATA DE FIM      ' LD00-DATAFIN. */
        ...
        {
            /*" -795- MOVE 'R400' TO WNR-EXEC-SQL. */
            _.Move("R400", WNR_EXEC_SQL);

            /*" -797- MOVE 'NAO' TO WS-ERRO-DATA */
            _.Move("NAO", WORK_AREA.WS_ERRO_DATA);

            /*" -799- IF WS-CRITICA-ANO LESS 1900 OR WS-CRITICA-ANO IS NOT NUMERIC */

            if (WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_ANO < 1900 || !WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_ANO.IsNumeric())
            {

                /*" -800- MOVE 'SIM' TO WS-ERRO-DATA */
                _.Move("SIM", WORK_AREA.WS_ERRO_DATA);

                /*" -801- GO TO R0400-99-SAIDA */
            ...
            /*" -806- IF WS-CRITICA-MES EQUAL ZEROS OR WS-CRITICA-MES GREATER 12 OR WS-CRITICA-MES IS NOT NUMERIC */
            ...
            if (WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_MES == 00 || WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_MES > 12 || !WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_MES.IsNumeric())
            {
                ...
                /*" -807- MOVE 'SIM' TO WS-ERRO-DATA */
                _.Move("SIM", WORK_AREA.WS_ERRO_DATA);

                /*" -808- GO TO R0400-99-SAIDA */
                /*Método Suprimido por falta de linha ou apenas EXIT nome: R0400_99_SAIDA*/ //GOTO
                return;
            ...
            /*" -812- IF WS-CRITICA-DIA EQUAL ZEROS OR WS-CRITICA-MES IS NOT NUMERIC */

            if (WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_DIA == 00 || !WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_MES.IsNumeric())
            {

                /*" -813- MOVE 'SIM' TO WS-ERRO-DATA */
                _.Move("SIM", WORK_AREA.WS_ERRO_DATA);

                /*" -814- GO TO R0400-99-SAIDA */
                /*Método Suprimido por falta de linha ou apenas EXIT nome: R0400_99_SAIDA*/ //GOTO
                return;
            ...
            /*" -819- IF WS-CRITICA-MES EQUAL 01 OR 03 OR 05 OR 07 OR 08 OR 10 OR 12 */

            if (WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_MES.In("01", "03", "05", "07", "08", "10", "12"))
            {

                /*" -820- IF WS-CRITICA-DIA GREATER 31 */

                if (WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_DIA > 31)
                {

                    /*" -821- MOVE 'SIM' TO WS-ERRO-DATA */
                    _.Move("SIM", WORK_AREA.WS_ERRO_DATA);

                    /*" -822- GO TO R0400-99-SAIDA */
                    /*Método Suprimido por falta de linha ou apenas EXIT nome: R0400_99_SAIDA*/ //GOTO
                    return;
            ...
            /*" -825- IF WS-CRITICA-MES EQUAL 04 OR 06 OR 09 OR 11 */

            if (WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_MES.In("04", "06", "09", "11"))
            {

                /*" -826- IF WS-CRITICA-DIA GREATER 30 */

                if (WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_DIA > 30)
                {

                    /*" -827- MOVE 'SIM' TO WS-ERRO-DATA */
                    _.Move("SIM", WORK_AREA.WS_ERRO_DATA);

                    /*" -828- GO TO R0400-99-SAIDA */
                    /*Método Suprimido por falta de linha ou apenas EXIT nome: R0400_99_SAIDA*/ //GOTO
                    return;
            ...
            if (WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_MES == 02)
            {
                ...
                else
                {
                    ...
                    /*" -843- IF WS-CRITICA-DIA GREATER 28 */

                    if (WORK_AREA.WS_DTH_CRITICA_R.WS_CRITICA_DIA > 28)
                    {

                        /*" -844- MOVE 'SIM' TO WS-ERRO-DATA */
                        _.Move("SIM", WORK_AREA.WS_ERRO_DATA);

                        /*" -845- GO TO R0400-99-SAIDA */
                        /*Método Suprimido por falta de linha ou apenas EXIT nome: R0400_99_SAIDA*/ //GOTO
                        return;
        ...
        {
            /*" -860- MOVE '090' TO WNR-EXEC-SQL. */
            _.Move("090", WNR_EXEC_SQL);

            /*" -861- DISPLAY '------------------------------------' */
            _.Display($"------------------------------------");

            /*" -862- DISPLAY '*** WHOST-DT-INICIO  ' WHOST-DT-INICIO */
            _.Display($"*** WHOST-DT-INICIO  {WHOST_DT_INICIO}");

            /*" -863- DISPLAY '*** WHOST-DT-FIM     ' WHOST-DT-FIM */
            _.Display($"*** WHOST-DT-FIM     {WHOST_DT_FIM}");

            /*" -865- DISPLAY '------------------------------------' */
            _.Display($"------------------------------------");

            /*" -900- PERFORM R0900_00_DECLARE_PROPOVA_DB_DECLARE_1 */

            R0900_00_DECLARE_PROPOVA_DB_DECLARE_1();

            /*" -902- PERFORM R0900_00_DECLARE_PROPOVA_DB_OPEN_1 */
        ...
        {
            /*" -900- EXEC SQL DECLARE TCOMIS CURSOR FOR SELECT B.NUM_APOLICE, B.CODSUBES, B.NRCERTIF, B.AGECOBR, B.DTQITBCO, DAYS (:V1SIST-DTMOVABE) - DAYS (B.DTQITBCO), B.CODUSU, B.CODCLIEN, B.OCOREND, C.IMPSEGUR, C.VLPREMIO, B.SITUACAO FROM SEGUROS.V0MOVIMENTO M, SEGUROS.V0PRODUTOSVG A, SEGUROS.V0PROPOSTAVA B, SEGUROS.V0COBERPROPVA C WHERE M.DATA_INCLUSAO IS NOT NULL AND M.DATA_INCLUSAO BETWEEN :WHOST-DT-INICIO AND :WHOST-DT-FIM AND M.COD_OPERACAO BETWEEN 100 AND 199 AND M.COD_USUARIO IN ( 'VA0601B' , 'VP0601B' , 'VA3118B' ) AND B.NRCERTIF = M.NUM_CERTIFICADO AND B.NUM_APOLICE = A.NUM_APOLICE AND B.CODSUBES = A.CODSUBES AND C.NRCERTIF = B.NRCERTIF AND C.OCORHIST = B.OCORHIST AND M.TIPO_SALARIO <> '0' END-EXEC. */
            TCOMIS = new VA0951B_TCOMIS(true);
        ...
        {
            /*" -902- EXEC SQL OPEN TCOMIS END-EXEC. */
        ...
        {
            /*" -1361- EXEC SQL DECLARE CERROSP CURSOR FOR SELECT SUBSTR(VALUE(T2.DES_ABREV_MSG_CRITICA, ' ' ),1,40) FROM SEGUROS.VG_CRITICA_PROPOSTA T1, SEGUROS.VG_DM_MSG_CRITICA T2 WHERE T1.NUM_CERTIFICADO = :WHOST-NRCERTIF AND T1.STA_CRITICA IN ( '0' , '1' , '2' , '4' , '5' , '8' ) AND T1.COD_MSG_CRITICA = T2.COD_MSG_CRITICA AND T2.COD_TP_MSG_CRITICA <> '3' ORDER BY T2.DES_ABREV_MSG_CRITICA WITH UR END-EXEC. */
            CERROSP = new VA0951B_CERROSP(true);
        ...
        {
            /*" -915- MOVE '091' TO WNR-EXEC-SQL. */
            _.Move("091", WNR_EXEC_SQL);

            /*" -929- PERFORM R0900_00_FETCH_PROPOVA_DB_FETCH_1 */
            ...
            if (DB.SQLCODE == 100)
            {
                ...
                /*" -933- MOVE 'S' TO WFIM-V0COMISICOB */
                _.Move("S", WORK_AREA.WFIM_V0COMISICOB);

                /*" -933- PERFORM R0900_00_FETCH_PROPOVA_DB_CLOSE_1 */

                R0900_00_FETCH_PROPOVA_DB_CLOSE_1();

                /*" -936- GO TO R0910-99-SAIDA. */
                /*Método Suprimido por falta de linha ou apenas EXIT nome: R0910_99_SAIDA*/ //GOTO
                return;
            }


            /*" -936- ADD 1 TO AC-LIDOS. */
        ...
        {
            /*" -929- EXEC SQL FETCH TCOMIS INTO :V0PROP-APOLICE, :V0PROP-CODSUBES, :V0PROP-NRCERTIF, :V0PROP-AGECOBR, :V0PROP-DTQITBCO, :WHOST-QTDIAS, :V0PROP-CODUSU, :V0PROP-CODCLIEN, :V0PROP-OCOREND, :V0PROP-IMPSEGUR, :V0PROP-VLPREMIO, :V0PROP-SITUACAO END-EXEC. */
        ...
        {
            /*" -933- EXEC SQL CLOSE TCOMIS END-EXEC */
        ...
        {
            /*" -949- PERFORM R1010-00-SELECT-CONVERSAO. */

            R1010_00_SELECT_CONVERSAO_SECTION();

            /*" -950- IF SQLCODE EQUAL ZEROS */

            if (DB.SQLCODE == 00)
            {

                /*" -951- MOVE CONV-NUM-SICOB TO WHOST-NUMTIT */
            ...
            else
            {
                ...
                /*" -954- MOVE V0PROP-NRCERTIF TO WHOST-NUMTIT. */
                _.Move(V0PROP_NRCERTIF, WHOST_NUMTIT);
            }


            /*" -957- INITIALIZE V0RCAPCOMP-NRRCAP V0RCAPCOMP-DATARCAP */
            _.Initialize(
                V0RCAPCOMP_NRRCAP
                , V0RCAPCOMP_DATARCAP
            );

            /*" -959- PERFORM R1020-00-SELECT-V0RCAP. */
            ...
            else
            {
                ...
                /*" -968- PERFORM R1030-00-SELECT-V0RCAPCOMP */
            ...
            /*" -972- PERFORM R1100-00-SELECT-V0CLIENTE. */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                /*" -974- ADD 1 TO AC-DESP-2 */
                AC_DESP_2.Value = AC_DESP_2 + 1;

                /*" -976- GO TO R1000-10-NEXT. */
            ...
            /*" -978- PERFORM R1200-00-SELECT-V0ENDERECO. */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                /*" -980- ADD 1 TO AC-DESP-3 */
                AC_DESP_3.Value = AC_DESP_3 + 1;

                /*" -982- GO TO R1000-10-NEXT. */
            ...
            /*" -984- PERFORM R1300-00-SELECT-V0AGENCIACEF. */

            R1300_00_SELECT_V0AGENCIACEF_SECTION();

            /*" -985- MOVE V0CLIE-NOME-RAZAO TO SVA-NOME-RAZAO. */
            _.Move(V0CLIE_NOME_RAZAO, REG_SVA0951B.SVA_NOME_RAZAO);

            /*" -986- MOVE V0CLIE-CGCCPF TO SVA-CGCCPF. */
            _.Move(V0CLIE_CGCCPF, REG_SVA0951B.SVA_CGCCPF);

            /*" -987- MOVE V0ENDE-ENDERECO TO SVA-ENDERECO. */
            _.Move(V0ENDE_ENDERECO, REG_SVA0951B.SVA_ENDERECO);

            /*" -988- MOVE V0ENDE-BAIRRO TO SVA-BAIRRO. */
            _.Move(V0ENDE_BAIRRO, REG_SVA0951B.SVA_BAIRRO);

            /*" -989- MOVE V0ENDE-CIDADE TO SVA-CIDADE. */
            _.Move(V0ENDE_CIDADE, REG_SVA0951B.SVA_CIDADE);

            /*" -990- MOVE V0ENDE-SIGLA-UF TO SVA-SIGLA-UF. */
            _.Move(V0ENDE_SIGLA_UF, REG_SVA0951B.SVA_SIGLA_UF);

            /*" -991- MOVE V0ENDE-CEP TO SVA-CEP. */
            _.Move(V0ENDE_CEP, REG_SVA0951B.SVA_CEP);

            /*" -992- MOVE V0ENDE-DDD TO SVA-DDD. */
            _.Move(V0ENDE_DDD, REG_SVA0951B.SVA_DDD);

            /*" -993- MOVE V0ENDE-TELEFONE TO SVA-TELEFONE. */
            _.Move(V0ENDE_TELEFONE, REG_SVA0951B.SVA_TELEFONE);

            /*" -994- MOVE V0PROP-APOLICE TO SVA-APOLICE. */
            _.Move(V0PROP_APOLICE, REG_SVA0951B.SVA_APOLICE);

            /*" -995- MOVE V0PROP-CODSUBES TO SVA-CODSUBES. */
            _.Move(V0PROP_CODSUBES, REG_SVA0951B.SVA_CODSUBES);

            /*" -996- MOVE V0PROP-NRCERTIF TO SVA-NRCERTIF. */
            _.Move(V0PROP_NRCERTIF, REG_SVA0951B.SVA_NRCERTIF);

            /*" -998- MOVE V0PROP-CODUSU TO SVA-CODUSU. */
            _.Move(V0PROP_CODUSU, REG_SVA0951B.SVA_CODUSU);

            /*" -999- MOVE V0PROP-AGECOBR TO SVA-AGECOBR. */
            _.Move(V0PROP_AGECOBR, REG_SVA0951B.SVA_AGECOBR);

            /*" -1000- MOVE V0MALHA-CDFONTE TO SVA-CDFONTE. */
            _.Move(V0MALHA_CDFONTE, REG_SVA0951B.SVA_CDFONTE);

            /*" -1001- MOVE V0MALHA-CDESCNEG TO SVA-CDESCNEG. */
            _.Move(V0MALHA_CDESCNEG, REG_SVA0951B.SVA_CDESCNEG);

            /*" -1002- MOVE V0PROP-IMPSEGUR TO SVA-IMPSEGUR. */
            _.Move(V0PROP_IMPSEGUR, REG_SVA0951B.SVA_IMPSEGUR);

            /*" -1003- MOVE V0PROP-VLPREMIO TO SVA-VLPREMIO. */
            _.Move(V0PROP_VLPREMIO, REG_SVA0951B.SVA_VLPREMIO);

            /*" -1004- MOVE V0PROP-SITUACAO TO SVA-SITUACAO. */
            _.Move(V0PROP_SITUACAO, REG_SVA0951B.SVA_SITUACAO);

            /*" -1006- MOVE WHOST-QTDIAS TO SVA-QTDIAS. */
            _.Move(WHOST_QTDIAS, REG_SVA0951B.SVA_QTDIAS);

            /*" -1007- IF V0RCAPCOMP-DATARCAP EQUAL SPACES */

            if (V0RCAPCOMP_DATARCAP.IsEmpty())
            {

                /*" -1008- MOVE V0PROP-DTQITBCO TO SVA-DTQITBCO */
                _.Move(V0PROP_DTQITBCO, REG_SVA0951B.SVA_DTQITBCO);
            ...
            else
            {
                ...
                /*" -1010- MOVE V0RCAPCOMP-DATARCAP TO SVA-DTQITBCO */
                _.Move(V0RCAPCOMP_DATARCAP, REG_SVA0951B.SVA_DTQITBCO);

                /*" -1012- END-IF */
            }


            /*" -1014- RELEASE REG-SVA0951B. */
            SVA0951B.Release(REG_SVA0951B);

            /*" -1014- PERFORM R3115-00-UPDATE-MOVIMENTO. */

            R3115_00_UPDATE_MOVIMENTO_SECTION();
        ...
        {
            /*" -1018- PERFORM R0900-00-FETCH-PROPOVA. */
        ...
        {
            /*" -1031- MOVE '110' TO WNR-EXEC-SQL. */
            _.Move("110", WNR_EXEC_SQL);

            /*" -1038- PERFORM R1100_00_SELECT_V0CLIENTE_DB_SELECT_1 */

            R1100_00_SELECT_V0CLIENTE_DB_SELECT_1();

            /*" -1041- IF SQLCODE NOT EQUAL ZEROS AND 100 */
        ...
        {
            /*" -1038- EXEC SQL SELECT NOME_RAZAO, CGCCPF INTO :V0CLIE-NOME-RAZAO, :V0CLIE-CGCCPF FROM SEGUROS.V0CLIENTE WHERE COD_CLIENTE = :V0PROP-CODCLIEN END-EXEC. */
        ...
        {
            /*" -1054- MOVE '101' TO WNR-EXEC-SQL. */
            _.Move("101", WNR_EXEC_SQL);

            /*" -1059- PERFORM R1010_00_SELECT_CONVERSAO_DB_SELECT_1 */

            R1010_00_SELECT_CONVERSAO_DB_SELECT_1();

            /*" -1062- IF SQLCODE NOT EQUAL ZEROS AND 100 */
            ...
            /*" -1065- IF SQLCODE EQUAL ZEROS */

            if (DB.SQLCODE == 00)
            {

                /*" -1065- MOVE CONV-NUM-SICOB TO CONV-NUMTIT. */
        ...
        {
            /*" -1059- EXEC SQL SELECT NUM_SICOB INTO :CONV-NUM-SICOB FROM SEGUROS.CONVERSAO_SICOB WHERE NUM_PROPOSTA_SIVPF = :V0PROP-NRCERTIF END-EXEC. */
        ...
        {
            /*" -1078- MOVE '102' TO WNR-EXEC-SQL. */
            _.Move("102", WNR_EXEC_SQL);

            /*" -1088- PERFORM R1020_00_SELECT_V0RCAP_DB_SELECT_1 */

            R1020_00_SELECT_V0RCAP_DB_SELECT_1();

            /*" -1091- IF SQLCODE NOT EQUAL ZEROS AND 100 */
        ...
        {
            /*" -1088- EXEC SQL SELECT FONTE , NRRCAP , SITUACAO INTO :V0RCAP-COD-FONTE , :V0RCAP-NUM-RCAP , :V0RCAP-SIT-REGISTRO FROM SEGUROS.V0RCAP WHERE NRTIT = :WHOST-NUMTIT END-EXEC. */
        ...
        {
            /*" -1104- MOVE '103' TO WNR-EXEC-SQL. */
            _.Move("103", WNR_EXEC_SQL);

            /*" -1114- PERFORM R1030_00_SELECT_V0RCAPCOMP_DB_SELECT_1 */

            R1030_00_SELECT_V0RCAPCOMP_DB_SELECT_1();

            /*" -1117- IF SQLCODE NOT EQUAL ZEROS AND 100 */
        ...
        {
            /*" -1114- EXEC SQL SELECT NRRCAP , DATARCAP INTO :V0RCAPCOMP-NRRCAP , :V0RCAPCOMP-DATARCAP FROM SEGUROS.V0RCAPCOMP WHERE FONTE = :V0RCAP-COD-FONTE AND NRRCAP = :V0RCAP-NUM-RCAP AND NRRCAPCO = 0 AND SITUACAO = '0' END-EXEC. */
        ...
        {
            /*" -1130- MOVE '120' TO WNR-EXEC-SQL. */
            _.Move("120", WNR_EXEC_SQL);

            /*" -1148- PERFORM R1200_00_SELECT_V0ENDERECO_DB_SELECT_1 */

            R1200_00_SELECT_V0ENDERECO_DB_SELECT_1();

            /*" -1151- IF SQLCODE NOT EQUAL ZEROS AND 100 */
        ...
        {
            /*" -1148- EXEC SQL SELECT ENDERECO, BAIRRO, CIDADE, SIGLA_UF, CEP, DDD, TELEFONE INTO :V0ENDE-ENDERECO, :V0ENDE-BAIRRO, :V0ENDE-CIDADE, :V0ENDE-SIGLA-UF, :V0ENDE-CEP, :V0ENDE-DDD, :V0ENDE-TELEFONE FROM SEGUROS.V0ENDERECOS WHERE COD_CLIENTE = :V0PROP-CODCLIEN AND OCORR_ENDERECO = :V0PROP-OCOREND END-EXEC. */
        ...
        {
            /*" -1164- MOVE '130' TO WNR-EXEC-SQL. */
            _.Move("130", WNR_EXEC_SQL);

            /*" -1173- PERFORM R1300_00_SELECT_V0AGENCIACEF_DB_SELECT_1 */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                if (DB.SQLCODE == 100)
                {
                    ...
                    /*" -1179- MOVE 99 TO V0MALHA-CDFONTE */
                    _.Move(99, V0MALHA_CDFONTE);

                    /*" -1180- MOVE 4172 TO V0MALHA-CDESCNEG */
        ...
        {
            /*" -1173- EXEC SQL SELECT B.COD_FONTE, A.COD_ESCNEG INTO :V0MALHA-CDFONTE, :V0MALHA-CDESCNEG FROM SEGUROS.V0AGENCIACEF A, SEGUROS.V0MALHACEF B WHERE A.COD_AGENCIA = :V0PROP-AGECOBR AND B.COD_SUREG = A.COD_ESCNEG END-EXEC. */
        ...
        {
            /*" -1194- MOVE SVA-APOLICE TO WS-APOLICE-ANT WHOST-APOLICE */
            _.Move(REG_SVA0951B.SVA_APOLICE, WORK_AREA.WS_APOLICE_ANT, WHOST_APOLICE);

            /*" -1197- MOVE SVA-CODSUBES TO WS-CODSUBES-ANT WHOST-CODSUBES. */
            _.Move(REG_SVA0951B.SVA_CODSUBES, WORK_AREA.WS_CODSUBES_ANT, WHOST_CODSUBES);

            /*" -1199- PERFORM R2500-00-SELECT-PRODUTO. */

            R2500_00_SELECT_PRODUTO_SECTION();

            /*" -1202- PERFORM R2100-00-PROCESSA-PRODUTO UNTIL SVA-APOLICE NOT EQUAL WS-APOLICE-ANT OR SVA-CODSUBES NOT EQUAL WS-CODSUBES-ANT OR WFIM-SORT EQUAL 'S' . */

            while (!(REG_SVA0951B.SVA_APOLICE != WORK_AREA.WS_APOLICE_ANT || REG_SVA0951B.SVA_CODSUBES != WORK_AREA.WS_CODSUBES_ANT || WORK_AREA.WFIM_SORT == "S"))
            {
        ...
        {
            /*" -1215- MOVE SVA-CDFONTE TO WS-FONTE-ANT. */
            _.Move(REG_SVA0951B.SVA_CDFONTE, WORK_AREA.WS_FONTE_ANT);

            /*" -1219- PERFORM R2200-00-PROCESSA-FONTE UNTIL SVA-APOLICE NOT EQUAL WS-APOLICE-ANT OR SVA-CODSUBES NOT EQUAL WS-CODSUBES-ANT OR SVA-CDFONTE NOT EQUAL WS-FONTE-ANT OR WFIM-SORT EQUAL 'S' . */

            while (!(REG_SVA0951B.SVA_APOLICE != WORK_AREA.WS_APOLICE_ANT || REG_SVA0951B.SVA_CODSUBES != WORK_AREA.WS_CODSUBES_ANT || REG_SVA0951B.SVA_CDFONTE != WORK_AREA.WS_FONTE_ANT || WORK_AREA.WFIM_SORT == "S"))
            {
        ...
        {
            /*" -1233- MOVE '-' TO LD01-TRA1 LD01-TRA1A */
            _.Move("-", LD01.LD01_TRA1, LD01.LD01_TRA1A);

            /*" -1254- MOVE ';' TO LD01-FIL1 LD01-FIL2 LD01-FIL2A LD01-FIL3 LD01-FIL3-A LD01-FIL3-A1 LD01-FIL3-A2 LD01-FIL3-B LD01-FIL3-C LD01-FIL3-D LD01-FIL3-E LD01-FIL3-F LD01-FIL3-G LD01-FIL3-H LD01-FIL3-I LD01-FIL4 LD01-FIL5 LD01-FIL6 LD01-FIL7 LD01-FIL8 */
            _.Move(";", LD01.LD01_FIL1, LD01.LD01_FIL2, LD01.LD01_FIL2A, LD01.LD01_FIL3, LD01.LD01_FIL3_A, LD01.LD01_FIL3_A1, LD01.LD01_FIL3_A2, LD01.LD01_FIL3_B, LD01.LD01_FIL3_C, LD01.LD01_FIL3_D, LD01.LD01_FIL3_E, LD01.LD01_FIL3_F, LD01.LD01_FIL3_G, LD01.LD01_FIL3_H, LD01.LD01_FIL3_I, LD01.LD01_FIL4, LD01.LD01_FIL5, LD01.LD01_FIL6, LD01.LD01_FIL7, LD01.LD01_FIL8);

            /*" -1255- IF SVA-CDESCNEG NOT EQUAL WS-COD-ESCNEG-ANT */

            if (REG_SVA0951B.SVA_CDESCNEG != WORK_AREA.WS_COD_ESCNEG_ANT)
            {

                /*" -1257- MOVE SVA-CDESCNEG TO WS-COD-ESCNEG-ANT WHOST-COD-ESCNEG */
                _.Move(REG_SVA0951B.SVA_CDESCNEG, WORK_AREA.WS_COD_ESCNEG_ANT, WHOST_COD_ESCNEG);
            ...
            _.Move(REG_SVA0951B.SVA_NOME_RAZAO, LD01.LD01_NOME_RAZAO);

            /*" -1262- MOVE SVA-CGCCPF TO LD01-CGCCPF */
            _.Move(REG_SVA0951B.SVA_CGCCPF, LD01.LD01_CGCCPF);

            /*" -1263- MOVE SVA-ENDERECO TO LD01-ENDERECO. */
            _.Move(REG_SVA0951B.SVA_ENDERECO, LD01.LD01_ENDERECO);

            /*" -1264- MOVE SVA-BAIRRO TO LD01-BAIRRO. */
            _.Move(REG_SVA0951B.SVA_BAIRRO, LD01.LD01_BAIRRO);

            /*" -1265- MOVE SVA-CIDADE TO LD01-CIDADE. */
            _.Move(REG_SVA0951B.SVA_CIDADE, LD01.LD01_CIDADE);

            /*" -1266- MOVE SVA-SIGLA-UF TO LD01-SIGLA-UF. */
            _.Move(REG_SVA0951B.SVA_SIGLA_UF, LD01.LD01_SIGLA_UF);

            /*" -1267- MOVE SVA-CEP TO LD01-CEP. */
            _.Move(REG_SVA0951B.SVA_CEP, LD01.LD01_CEP);

            /*" -1268- MOVE SVA-DDD TO LD01-DDD. */
            _.Move(REG_SVA0951B.SVA_DDD, LD01.LD01_DDD);

            /*" -1269- MOVE SVA-TELEFONE TO LD01-TELEFONE. */
            _.Move(REG_SVA0951B.SVA_TELEFONE, LD01.LD01_TELEFONE);

            /*" -1270- MOVE SVA-CDESCNEG TO LD01-COD-ESCNEG */
            _.Move(REG_SVA0951B.SVA_CDESCNEG, LD01.LD01_COD_ESCNEG);
            ...
            _.Move(REG_SVA0951B.SVA_CDFONTE, LD01.LD01_CODFILIAL);

            /*" -1273- MOVE TBFT-NMFONTE(SVA-CDFONTE) TO LD01-NOMFILIAL */
            _.Move(TABELA_FONTES1.TAB_FTE1[REG_SVA0951B.SVA_CDFONTE].TBFT_NMFONTE, LD01.LD01_NOMFILIAL);

            /*" -1274- MOVE SVA-NRCERTIF TO LD01-NRCERTIF */
            _.Move(REG_SVA0951B.SVA_NRCERTIF, LD01.LD01_NRCERTIF);

            /*" -1275- MOVE SVA-CODUSU TO LD01-CODUSU */
            _.Move(REG_SVA0951B.SVA_CODUSU, LD01.LD01_CODUSU);

            /*" -1276- MOVE SVA-DTQITBCO (1:4) TO LD01-DTQITBCO(7:4) */
            _.MoveAtPosition(REG_SVA0951B.SVA_DTQITBCO.Substring(1, 4), LD01.LD01_DTQITBCO, 7, 4);

            /*" -1277- MOVE '/' TO LD01-DTQITBCO(3:1) */
            _.MoveAtPosition("/", LD01.LD01_DTQITBCO, 3, 1);

            /*" -1278- MOVE SVA-DTQITBCO (6:2) TO LD01-DTQITBCO(4:2) */
            _.MoveAtPosition(REG_SVA0951B.SVA_DTQITBCO.Substring(6, 2), LD01.LD01_DTQITBCO, 4, 2);

            /*" -1279- MOVE '/' TO LD01-DTQITBCO(6:1) */
            _.MoveAtPosition("/", LD01.LD01_DTQITBCO, 6, 1);

            /*" -1280- MOVE SVA-DTQITBCO (9:2) TO LD01-DTQITBCO(1:2) */
            _.MoveAtPosition(REG_SVA0951B.SVA_DTQITBCO.Substring(9, 2), LD01.LD01_DTQITBCO, 1, 2);

            /*" -1281- MOVE SVA-AGECOBR TO LD01-AGECOBR */
            _.Move(REG_SVA0951B.SVA_AGECOBR, LD01.LD01_AGECOBR);

            /*" -1282- MOVE SVA-VLPREMIO TO LD01-VLPREMIO */
            _.Move(REG_SVA0951B.SVA_VLPREMIO, LD01.LD01_VLPREMIO);

            /*" -1283- MOVE SVA-IMPSEGUR TO LD01-IMPSEGUR */
            _.Move(REG_SVA0951B.SVA_IMPSEGUR, LD01.LD01_IMPSEGUR);

            /*" -1284- MOVE SVA-QTDIAS TO LD01-QTDIAS */
            _.Move(REG_SVA0951B.SVA_QTDIAS, LD01.LD01_QTDIAS);

            /*" -1286- MOVE 'EMISSAO AUTOMATICA' TO LD01-DESCR-ERRO. */
            _.Move("EMISSAO AUTOMATICA", LD01.LD01_DESCR_ERRO);

            /*" -1286- WRITE REG-AVA0951B FROM LD01. */
            _.Move(LD01.GetMoveValues(), REG_AVA0951B);
        ...
        {
            /*" -1304- PERFORM R2280-00-DECLARE-ERROSPROP. */

            R2280_00_DECLARE_ERROSPROP_SECTION();

            /*" -1305- MOVE SPACES TO WFIM-V0ERROSPROP. */
            _.Move("", WORK_AREA.WFIM_V0ERROSPROP);

            /*" -1307- PERFORM R2290-00-FETCH-ERROSPROP. */

            R2290_00_FETCH_ERROSPROP_SECTION();

            /*" -1308- IF WFIM-V0ERROSPROP EQUAL 'SIM' */

            if (WORK_AREA.WFIM_V0ERROSPROP == "SIM")
            {

                /*" -1309- MOVE 'PROPOSTA SEM ERRO' TO LD01-DESCR-ERRO */
                _.Move("PROPOSTA SEM ERRO", LD01.LD01_DESCR_ERRO);

                /*" -1310- WRITE REG-AVA0951B FROM LD01 */
                _.Move(LD01.GetMoveValues(), REG_AVA0951B);

                AVA0951B.Write(REG_AVA0951B.GetMoveValues().ToString());

                /*" -1312- GO TO R2210-99-SAIDA. */
                /*Método Suprimido por falta de linha ou apenas EXIT nome: R2210_99_SAIDA*/ //GOTO
                return;
            }


            /*" -1312- PERFORM R2220-00-PROCESSA-ERROSPROP. */
        ...
        {
            /*" -1325- MOVE V0ERROSP-DESCR-ERRO TO LD01-DESCR-ERRO. */
            _.Move(V0ERROSP_DESCR_ERRO, LD01.LD01_DESCR_ERRO);

            /*" -1325- WRITE REG-AVA0951B FROM LD01. */
            _.Move(LD01.GetMoveValues(), REG_AVA0951B);
        ...
        {
            /*" -1340- MOVE '228' TO WNR-EXEC-SQL. */
            _.Move("228", WNR_EXEC_SQL);

            /*" -1351- MOVE SVA-NRCERTIF TO WHOST-NRCERTIF. */
            _.Move(REG_SVA0951B.SVA_NRCERTIF, WHOST_NRCERTIF);

            /*" -1361- PERFORM R2280_00_DECLARE_ERROSPROP_DB_DECLARE_1 */

            R2280_00_DECLARE_ERROSPROP_DB_DECLARE_1();

            /*" -1363- PERFORM R2280_00_DECLARE_ERROSPROP_DB_OPEN_1 */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                /*" -1367- DISPLAY 'R2280 - PROBLEMAS DECLARE (ERROSPROP) ..' */
                _.Display($"R2280 - PROBLEMAS DECLARE (ERROSPROP) ..");

                /*" -1368- DISPLAY 'SQLCODE - ' SQLCODE */
        ...
        {
            /*" -1363- EXEC SQL OPEN CERROSP END-EXEC. */
        ...
        {
            /*" -1450- EXEC SQL DECLARE CFONTES CURSOR FOR SELECT FONTE, NOMEFTE FROM SEGUROS.V0FONTE ORDER BY FONTE END-EXEC. */
            CFONTES = new VA0951B_CFONTES(false);
        ...
        {
            /*" -1381- MOVE '229' TO WNR-EXEC-SQL. */
            _.Move("229", WNR_EXEC_SQL);

            /*" -1383- PERFORM R2290_00_FETCH_ERROSPROP_DB_FETCH_1 */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                if (DB.SQLCODE == 100)
                {
                    ...
                    /*" -1389- MOVE 'SIM' TO WFIM-V0ERROSPROP */
                ...
                else
                {
                    ...
                    /*" -1391- DISPLAY 'R2290 - PROBLEMAS DECLARE (ERROSPROP) ..' */
            ...
            /*" -1394- PERFORM R2290_00_FETCH_ERROSPROP_DB_CLOSE_1 */
        ...
        {
            /*" -1383- EXEC SQL FETCH CERROSP INTO :V0ERROSP-DESCR-ERRO END-EXEC. */
        ...
        {
            /*" -1394- EXEC SQL CLOSE CERROSP END-EXEC. */
        ...
        {
            /*" -1407- RETURN SVA0951B AT END */
            try
            {
                SVA0951B.Return(REG_SVA0951B, () =>
                {

                    /*" -1407- MOVE 'S' TO WFIM-SORT. */
        ...
        {
            /*" -1420- MOVE '250' TO WNR-EXEC-SQL. */
            _.Move("250", WNR_EXEC_SQL);

            /*" -1426- PERFORM R2500_00_SELECT_PRODUTO_DB_SELECT_1 */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                if (DB.SQLCODE == 100)
                {
                    ...
                    /*" -1431- MOVE 'PRODUTO INEXISTENTE' TO V0PROD-NOMPRODU */
        ...
        {
            /*" -1426- EXEC SQL SELECT NOMPRODU INTO :V0PROD-NOMPRODU FROM SEGUROS.V0PRODUTOSVG WHERE NUM_APOLICE = :WHOST-APOLICE AND CODSUBES = :WHOST-CODSUBES END-EXEC. */
        ...
        {
            /*" -1445- MOVE '310' TO WNR-EXEC-SQL. */
            _.Move("310", WNR_EXEC_SQL);

            /*" -1450- PERFORM R3100_00_DECLARE_FONTES_DB_DECLARE_1 */

            R3100_00_DECLARE_FONTES_DB_DECLARE_1();

            /*" -1452- PERFORM R3100_00_DECLARE_FONTES_DB_OPEN_1 */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                /*" -1456- DISPLAY 'R3100 - PROBLEMAS DECLARE (V0FONTE  ) ..' */
                _.Display($"R3100 - PROBLEMAS DECLARE (V0FONTE  ) ..");

                /*" -1457- DISPLAY 'SQLCODE - ' SQLCODE */
                _.Display($"SQLCODE - {DB.SQLCODE}");

                /*" -1457- GO TO R9999-00-ROT-ERRO. */
        ...
        {
            /*" -1452- EXEC SQL OPEN CFONTES END-EXEC. */
        ...
        {
            /*" -1599- EXEC SQL DECLARE CESCNEG CURSOR FOR SELECT COD_ESCNEG, REGIAO_ESCNEG, COD_FONTE FROM SEGUROS.V0ESCNEG ORDER BY COD_ESCNEG END-EXEC. */
            CESCNEG = new VA0951B_CESCNEG(false);
        ...
        {
            /*" -1470- MOVE '311' TO WNR-EXEC-SQL. */
            _.Move("311", WNR_EXEC_SQL);

            /*" -1473- PERFORM R3110_00_FETCH_FONTES_DB_FETCH_1 */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                /*" -1477- IF SQLCODE EQUAL 100 */

                if (DB.SQLCODE == 100)
                {

                    /*" -1478- MOVE 'S' TO WFIM-FONTES */
                    _.Move("S", WORK_AREA.WFIM_FONTES);

                    /*" -1478- PERFORM R3110_00_FETCH_FONTES_DB_CLOSE_1 */
                ...
                else
                {
                    ...
                    /*" -1480- PERFORM R3110_00_FETCH_FONTES_DB_CLOSE_2 */

                    R3110_00_FETCH_FONTES_DB_CLOSE_2();

                    /*" -1482- DISPLAY '3110 - (PROBLEMAS NO FETCH CFONTES  ) ..' */
                    _.Display($"3110 - (PROBLEMAS NO FETCH CFONTES  ) ..");

                    /*" -1483- DISPLAY 'SQLCODE - ' SQLCODE */
                    _.Display($"SQLCODE - {DB.SQLCODE}");
        ...
        {
            /*" -1473- EXEC SQL FETCH CFONTES INTO :V0FONT-CODFTE, :V0FONT-NOMEFTE END-EXEC. */
        ...
        {
            /*" -1478- EXEC SQL CLOSE CFONTES END-EXEC */
        ...
        {
            /*" -1480- EXEC SQL CLOSE CFONTES END-EXEC */
        ...
        [StopWatch]
        /*" R3115-00-UPDATE-MOVIMENTO-SECTION */
        private void R3115_00_UPDATE_MOVIMENTO_SECTION()
        {
            /*" -1495- MOVE '3115' TO WNR-EXEC-SQL. */
            _.Move("3115", WNR_EXEC_SQL);

            /*" -1501- PERFORM R3115_00_UPDATE_MOVIMENTO_DB_UPDATE_1 */

            R3115_00_UPDATE_MOVIMENTO_DB_UPDATE_1();

            /*" -1504- IF (SQLCODE NOT EQUAL ZEROS AND +100) */

            if ((!DB.SQLCODE.In("00", "+100")))
            {

                /*" -1505- DISPLAY 'ERRO NA ATUALIZACAO MOVIMENTO ' V0PROP-NRCERTIF */
                _.Display($"ERRO NA ATUALIZACAO MOVIMENTO {V0PROP_NRCERTIF}");
        ...
        [StopWatch]
        /*" R3115-00-UPDATE-MOVIMENTO-DB-UPDATE-1 */
        public void R3115_00_UPDATE_MOVIMENTO_DB_UPDATE_1()
        {
            /*" -1501- EXEC SQL UPDATE SEGUROS.V0MOVIMENTO SET TIPO_SALARIO = '0' WHERE NUM_CERTIFICADO = :V0PROP-NRCERTIF AND COD_OPERACAO BETWEEN 100 AND 199 AND COD_USUARIO IN ( 'VA0601B' , 'VP0601B' , 'VA3118B' ) END-EXEC. */

            var r3115_00_UPDATE_MOVIMENTO_DB_UPDATE_1_Update1 = new R3115_00_UPDATE_MOVIMENTO_DB_UPDATE_1_Update1()
            {
                V0PROP_NRCERTIF = V0PROP_NRCERTIF.ToString(),
            };

            R3115_00_UPDATE_MOVIMENTO_DB_UPDATE_1_Update1.Execute(r3115_00_UPDATE_MOVIMENTO_DB_UPDATE_1_Update1);

        }
        /*Método Suprimido por falta de linha ou apenas EXIT nome: R3115_99_SAIDA*/

        [StopWatch]
        /*" R3116-00-UPDATE-PREP-MVMTO-SECTION */
        private void R3116_00_UPDATE_PREP_MVMTO_SECTION()
        {
            /*" -1518- MOVE '3116' TO WNR-EXEC-SQL. */
            _.Move("3116", WNR_EXEC_SQL);

            /*" -1527- PERFORM R3116_00_UPDATE_PREP_MVMTO_DB_UPDATE_1 */

            R3116_00_UPDATE_PREP_MVMTO_DB_UPDATE_1();

            /*" -1530- IF (SQLCODE NOT EQUAL ZEROS AND +100) */

            if ((!DB.SQLCODE.In("00", "+100")))
            {

                /*" -1532- DISPLAY 'R3116-ERRO ATUALIZACAO MVMNTO ' WHOST-DT-INICIO '  ' WHOST-DT-FIM */

                $"R3116-ERRO ATUALIZACAO MVMNTO {WHOST_DT_INICIO}  {WHOST_DT_FIM}"
                .Display();
        ...
        [StopWatch]
        /*" R3116-00-UPDATE-PREP-MVMTO-DB-UPDATE-1 */
        public void R3116_00_UPDATE_PREP_MVMTO_DB_UPDATE_1()
        {
            /*" -1527- EXEC SQL UPDATE SEGUROS.V0MOVIMENTO SET TIPO_SALARIO = ' ' WHERE DATA_INCLUSAO IS NOT NULL AND DATA_INCLUSAO BETWEEN :WHOST-DT-INICIO AND :WHOST-DT-FIM AND COD_OPERACAO BETWEEN 100 AND 199 AND COD_USUARIO IN ( 'VA0601B' , 'VP0601B' , 'VA3118B' ) AND TIPO_SALARIO = '0' END-EXEC. */

            var r3116_00_UPDATE_PREP_MVMTO_DB_UPDATE_1_Update1 = new R3116_00_UPDATE_PREP_MVMTO_DB_UPDATE_1_Update1()
            {
                WHOST_DT_INICIO = WHOST_DT_INICIO.ToString(),
                WHOST_DT_FIM = WHOST_DT_FIM.ToString(),
            };

            R3116_00_UPDATE_PREP_MVMTO_DB_UPDATE_1_Update1.Execute(r3116_00_UPDATE_PREP_MVMTO_DB_UPDATE_1_Update1);

        }
        /*Método Suprimido por falta de linha ou apenas EXIT nome: R3116_99_SAIDA*/

        [StopWatch]
        /*" R3120-00-CARREGA-FONTES-SECTION */
        private void R3120_00_CARREGA_FONTES_SECTION()
        {
            /*" -1546- MOVE '311' TO WNR-EXEC-SQL. */
            _.Move("311", WNR_EXEC_SQL);

            /*" -1547- IF V0FONT-CODFTE LESS 100 */

            if (V0FONT_CODFTE < 100)
            {

                /*" -1550- MOVE V0FONT-NOMEFTE TO TBFT-NMFONTE (V0FONT-CODFTE). */
                _.Move(V0FONT_NOMEFTE, TABELA_FONTES1.TAB_FTE1[V0FONT_CODFTE].TBFT_NMFONTE);
            }


            /*" -1550- PERFORM R3110-00-FETCH-FONTES. */
        ...
        {
            /*" -1563- MOVE '319' TO WNR-EXEC-SQL. */
            _.Move("319", WNR_EXEC_SQL);

            /*" -1572- PERFORM R3190_00_SELECT_ESCNEG_DB_SELECT_1 */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                if (DB.SQLCODE == 100)
                {
                    ...
                    /*" -1577- MOVE 'NAO CADASTRADO' TO V0ESCN-NOMEESC */
                ...
                else
                {
                    ...
                    /*" -1579- DISPLAY 'R3190 - PROBLEMAS SELECT  (V0ESCNEG ) ..' */
                    _.Display($"R3190 - PROBLEMAS SELECT  (V0ESCNEG ) ..");

                    /*" -1580- DISPLAY 'SQLCODE - ' SQLCODE */
                    _.Display($"SQLCODE - {DB.SQLCODE}");
        ...
        {
            /*" -1572- EXEC SQL SELECT COD_ESCNEG, REGIAO_ESCNEG, COD_FONTE INTO :V0ESCN-CODESC, :V0ESCN-NOMEESC, :V0ESCN-FONTE FROM SEGUROS.V0ESCNEG WHERE COD_ESCNEG = :WHOST-COD-ESCNEG END-EXEC. */
        ...
        {
            /*" -1593- MOVE '320' TO WNR-EXEC-SQL. */
            _.Move("320", WNR_EXEC_SQL);

            /*" -1599- PERFORM R3200_00_DECLARE_ESCNEG_DB_DECLARE_1 */

            R3200_00_DECLARE_ESCNEG_DB_DECLARE_1();

            /*" -1601- PERFORM R3200_00_DECLARE_ESCNEG_DB_OPEN_1 */

            R3200_00_DECLARE_ESCNEG_DB_OPEN_1();

            /*" -1604- IF SQLCODE NOT EQUAL ZEROS */

            if (DB.SQLCODE != 00)
            {

                /*" -1605- DISPLAY 'R3200 - PROBLEMAS DECLARE (V0ESCNEG ) ..' */
                _.Display($"R3200 - PROBLEMAS DECLARE (V0ESCNEG ) ..");

                /*" -1606- DISPLAY 'SQLCODE - ' SQLCODE */
                _.Display($"SQLCODE - {DB.SQLCODE}");

                /*" -1606- GO TO R9999-00-ROT-ERRO. */
        ...
        {
            /*" -1601- EXEC SQL OPEN CESCNEG END-EXEC. */
        ...
        {
            /*" -1619- MOVE '321' TO WNR-EXEC-SQL. */
            _.Move("321", WNR_EXEC_SQL);

            /*" -1623- PERFORM R3210_00_FETCH_ESCNEG_DB_FETCH_1 */
            ...
            if (DB.SQLCODE != 00)
            {
                ...
                if (DB.SQLCODE == 100)
                {
                    ...
                    /*" -1628- MOVE 'S' TO WFIM-ESCNEG */
                    _.Move("S", WORK_AREA.WFIM_ESCNEG);

                    /*" -1628- PERFORM R3210_00_FETCH_ESCNEG_DB_CLOSE_1 */
                ...
                else
                {
                    ...
                    /*" -1630- PERFORM R3210_00_FETCH_ESCNEG_DB_CLOSE_2 */

                    R3210_00_FETCH_ESCNEG_DB_CLOSE_2();

                    /*" -1632- DISPLAY '3210 - (PROBLEMAS NO FETCH CESCNEG  ) ..' */
                    _.Display($"3210 - (PROBLEMAS NO FETCH CESCNEG  ) ..");

                    /*" -1633- DISPLAY 'SQLCODE - ' SQLCODE */
                    _.Display($"SQLCODE - {DB.SQLCODE}");
        ...
        {
            /*" -1623- EXEC SQL FETCH CESCNEG INTO :V0ESCN-CODESC, :V0ESCN-NOMEESC, :V0ESCN-FONTE END-EXEC. */
        ...
        {
            /*" -1628- EXEC SQL CLOSE CESCNEG END-EXEC */
        ...
        {
            /*" -1630- EXEC SQL CLOSE CESCNEG END-EXEC */
        ...
        private void R9800_00_ENCERRA_SEM_SOLIC_SECTION()
        {
            ...
            /*" -1649- DISPLAY '*   VA0951B - GERA RELATORIO MULTPREMIADO  *' */
            _.Display($"*   VA0951B - GERA RELATORIO MULTPREMIADO  *");
            ...
            /*" -1652- DISPLAY '*             NAO EXISTE PRODUCAO PARA     *' */
            _.Display($"*             NAO EXISTE PRODUCAO PARA     *");

            /*" -1653- DISPLAY '*             O PERIODO PEDIDO.            *' */
        ...
        {
            /*" -1668- MOVE SQLCODE TO WSQLCODE. */
            _.Move(DB.SQLCODE, WSQLCODE);

            /*" -1670- DISPLAY WABEND */
            _.Display(FILLER_3.WABEND);

            /*" -1670- EXEC SQL WHENEVER SQLERROR CONTINUE END-EXEC. */

            /*" -1672- EXEC SQL ROLLBACK WORK END-EXEC. */

            DatabaseConnection.Instance.RollbackTransaction();

            /*" -1676- MOVE 99 TO RETURN-CODE. */
