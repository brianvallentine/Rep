...
namespace Sias.Outros.DB2.CS1301B
{
    public class R0100_00_SELECT_SISTEMAS_DB_SELECT_1_Query1 : QueryBasis<R0100_00_SELECT_SISTEMAS_DB_SELECT_1_Query1>
    {
        string GetQuery()
        {
            ...
            (DATA_MOV_ABERTO + 6 DAYS) ,
            (DATA_MOV_ABERTO - 1 DAY) ,
            CURRENT DATE
            INTO :SISTEMAS-DATA-MOV-ABERTO ,
            :SISTEMAS-DATA-MOV-6D ,
            :SISTEMAS-DATA-MOV-1D ,
            :WHOST-DTCURRENT
            FROM SEGUROS.SISTEMAS
            WHERE IDE_SISTEMA = 'CS'
            END-EXEC.
            */
            #endregion
            var query = @$"
        ...
        public string SISTEMAS_DATA_MOV_6D { get; set; }
        public string SISTEMAS_DATA_MOV_1D { get; set; }
        ...
        public override R0100_00_SELECT_SISTEMAS_DB_SELECT_1_Query1 OpenData(List<KeyValuePair<string, object>> result)
        {
            ...
            dta.SISTEMAS_DATA_MOV_6D = result[i++].Value?.ToString();
            dta.SISTEMAS_DATA_MOV_1D = result[i++].Value?.ToString();
