using System;
using IA_ConverterCommons;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using System.Threading.Tasks;
using System.Linq;
using _ = IA_ConverterCommons.Statements;
using DB = IA_ConverterCommons.DatabaseBasis;

namespace Sias.PessoaFisica.DB2.PF0605B
{
    public class R0195_BUSCA_DADOS_HISPROFI_DB_SELECT_2_Query1 : QueryBasis<R0195_BUSCA_DADOS_HISPROFI_DB_SELECT_2_Query1>
    {
        string GetQuery()
        {
            #region SQL_SOURCE
            /*EXEC SQL
            SELECT DATA_SITUACAO
            INTO :PROPFIDH-DATA-SITUACAO
            FROM SEGUROS.HIST_PROP_FIDELIZ
            WHERE NUM_IDENTIFICACAO =
            :PROPFIDH-NUM-IDENTIFICACAO
            AND SIT_PROPOSTA =
            :PROPFIDH-SIT-PROPOSTA
            AND SIT_MOTIVO_SIVPF =
            :PROPFIDH-SIT-MOTIVO-SIVPF
            AND DATA_SITUACAO =
            :PROPFIDH-DATA-SITUACAO
            WITH UR
            END-EXEC
            */
            #endregion
            var query = @$"
				SELECT DATA_SITUACAO
											FROM SEGUROS.HIST_PROP_FIDELIZ
											WHERE NUM_IDENTIFICACAO =
											'{this.PROPFIDH_NUM_IDENTIFICACAO}'
											AND SIT_PROPOSTA =
											'{this.PROPFIDH_SIT_PROPOSTA}'
											AND SIT_MOTIVO_SIVPF =
											'{this.PROPFIDH_SIT_MOTIVO_SIVPF}'
											AND DATA_SITUACAO =
											'{this.PROPFIDH_DATA_SITUACAO}'
											WITH UR";

            return query;
        }
        public string PROPFIDH_DATA_SITUACAO { get; set; }
        public string PROPFIDH_NUM_IDENTIFICACAO { get; set; }
        public string PROPFIDH_SIT_MOTIVO_SIVPF { get; set; }
        public string PROPFIDH_SIT_PROPOSTA { get; set; }

        public static R0195_BUSCA_DADOS_HISPROFI_DB_SELECT_2_Query1 Execute(R0195_BUSCA_DADOS_HISPROFI_DB_SELECT_2_Query1 r0195_BUSCA_DADOS_HISPROFI_DB_SELECT_2_Query1)
        {
            var ths = r0195_BUSCA_DADOS_HISPROFI_DB_SELECT_2_Query1;
            ths.SetQuery(ths.GetQuery());

            ths.Open();
            var isFetch = ths.Fetch();

            return isFetch ? ths : null;
        }

        public override R0195_BUSCA_DADOS_HISPROFI_DB_SELECT_2_Query1 OpenData(List<KeyValuePair<string, object>> result)
        {
            var dta = new R0195_BUSCA_DADOS_HISPROFI_DB_SELECT_2_Query1();
            var i = 0;
            dta.PROPFIDH_DATA_SITUACAO = result[i++].Value?.ToString();
            return dta;
        }

    }
}