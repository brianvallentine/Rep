...
namespace Sias.Cosseguro.DB2.AC0011B
{
    public class R0200_00_CHECA_EXECUCAO_DB_SELECT_1_Query1 : QueryBasis<R0200_00_CHECA_EXECUCAO_DB_SELECT_1_Query1>
    {
        string GetQuery()
        {
            ...
            /*EXEC SQL
            SELECT COUNT(*)
            INTO :WSHOST-QTDE-REG
            FROM SEGUROS.V1COSSEG_HISTPRE A,
            SEGUROS.V0APOLICE B
            WHERE A.DTMOVTO = :V0SIST-DTMOVABE
            AND A.TIPSGU = '1'
            AND A.OPERACAO < 0600
            AND B.NUM_APOLICE = A.NUM_APOLICE
            AND B.TIPSGU = '1'
            AND B.ORGAO NOT IN (0010,0300)
            END-EXEC.
            */
            #endregion
            var query = @$"
				SELECT COUNT(*)
							...
							,
											SEGUROS.V0APOLICE B
											WHERE A.DTMOVTO = '{this.V0SIST_DTMOVABE}'
											AND A.TIPSGU = '1'
											AND A.OPERACAO < 0600
											AND B.NUM_APOLICE = A.NUM_APOLICE
											AND B.TIPSGU = '1'
        ...
        public string WSHOST_QTDE_REG { get; set; }
        ...
        public override R0200_00_CHECA_EXECUCAO_DB_SELECT_1_Query1 OpenData(List<KeyValuePair<string, object>> result)
        {
            ...
            var i = 0;
            dta.WSHOST_QTDE_REG = result[i++].Value?.ToString();
            return dta;
        }
