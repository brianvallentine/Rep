using System;
using IA_ConverterCommons;
using System.Collections.Generic;
using System.Text.Json.Serialization;
using System.Threading.Tasks;
using System.Linq;
using _ = IA_ConverterCommons.Statements;
using DB = IA_ConverterCommons.DatabaseBasis;

namespace Sias.Cosseguro.DB2.AC0017B
{
    public class AC0017B_V1MESTSINI : QueryBasis<AC0017B_V1MESTSINI>
    {
        [JsonIgnore] public bool JustACursor { get; set; } = false;

        public delegate string GetQueryDelegateHandler();
        public event GetQueryDelegateHandler GetQueryEvent;

        //ESTE CONSTRUTOR NÃO DEVE SER USADO ( CUIDADO )
        public AC0017B_V1MESTSINI() { IsCursor = true; }

        public AC0017B_V1MESTSINI(bool justACursor)
        {
            JustACursor = justACursor;
            IsCursor = true;
        }

        public string V1MSIN_NUM_SINI { get; set; }
        public string V1MSIN_NUM_APOL { get; set; }
        public string V1MSIN_NRENDOS { get; set; }
        public string V1MSIN_COD_MOEDA { get; set; }
        public string V1MSIN_TIPREG { get; set; }
        public string V1MSIN_RAMO { get; set; }
        public string V1MSIN_CODCAU { get; set; }
        public string V1MSIN_DATORR { get; set; }
        public string V1MSIN_PCTRES { get; set; }
        public string V1HSIN_OCORHIST { get; set; }
        public string V1HSIN_OPERACAO { get; set; }
        public string V1HSIN_DTMOVTO { get; set; }
        public string V1HSIN_VAL_OPER { get; set; }
        public string V1HSIN_TIPREG { get; set; }
        public string V1HSIN_SITUACAO { get; set; }
        public string GEOPERAC_FUNCAO_OPERACAO { get; set; }
        public string GEOPERAC_IND_TIPO_FUNCAO { get; set; }

        public new void Open()
        {
            if (!IsProcedure)
                SetQuery(GetQueryEvent());
            base.Open();
        }


        public new bool Fetch()
        {
            if (!JustACursor)
            {
                var idx = CurrentIndex;
                Open();
                CurrentIndex = idx > -1 ? idx : 0;
            }

            return base.Fetch();
        }


        public override AC0017B_V1MESTSINI OpenData(List<KeyValuePair<string, object>> result)
        {
            var dta = new AC0017B_V1MESTSINI();
            var i = 0;

            dta.V1MSIN_NUM_SINI = result[i++].Value?.ToString();
            dta.V1MSIN_NUM_APOL = result[i++].Value?.ToString();
            dta.V1MSIN_NRENDOS = result[i++].Value?.ToString();
            dta.V1MSIN_COD_MOEDA = result[i++].Value?.ToString();
            dta.V1MSIN_TIPREG = result[i++].Value?.ToString();
            dta.V1MSIN_RAMO = result[i++].Value?.ToString();
            dta.V1MSIN_CODCAU = result[i++].Value?.ToString();
            dta.V1MSIN_DATORR = result[i++].Value?.ToString();
            dta.V1MSIN_PCTRES = result[i++].Value?.ToString();
            dta.V1HSIN_OCORHIST = result[i++].Value?.ToString();
            dta.V1HSIN_OPERACAO = result[i++].Value?.ToString();
            dta.V1HSIN_DTMOVTO = result[i++].Value?.ToString();
            dta.V1HSIN_VAL_OPER = result[i++].Value?.ToString();
            dta.V1HSIN_TIPREG = result[i++].Value?.ToString();
            dta.V1HSIN_SITUACAO = result[i++].Value?.ToString();
            dta.GEOPERAC_FUNCAO_OPERACAO = result[i++].Value?.ToString();
            dta.GEOPERAC_IND_TIPO_FUNCAO = result[i++].Value?.ToString();

            return dta;
        }

    }
}