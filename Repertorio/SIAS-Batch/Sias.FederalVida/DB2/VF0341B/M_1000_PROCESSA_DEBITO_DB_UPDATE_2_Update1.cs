...
namespace Sias.FederalVida.DB2.VF0341B
{
    public class M_1000_PROCESSA_DEBITO_DB_UPDATE_2_Update1 : QueryBasis<M_1000_PROCESSA_DEBITO_DB_UPDATE_2_Update1>
    {
        ...
        private VF0341B_DEBITO CurrentOf { get; set; }

        public M_1000_PROCESSA_DEBITO_DB_UPDATE_2_Update1(VF0341B_DEBITO currentOf)
        {
            CurrentOf = currentOf;
        }
        ...
        string GetQuery()
        {
            var query = @$"
				...
				WHERE
				(
					CODCONV = 6131 AND SITUACAO = '0' AND TIPLANC = '2' AND DTVENCTO <= '{this.SIST_DTMAXDEB}'
				)
