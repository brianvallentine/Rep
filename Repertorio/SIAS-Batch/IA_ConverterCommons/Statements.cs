...
public static class Statements
{
    ...
    public static void MoveAtPosition(this object from, VarBasis to, long start, long length, params VarBasis[] tos)
    {
        ...
        //Removido o = para que nao ocorra Execpiton de char Vazio
        //for (long i = startIndex, j = 0; i <= length - 1 && i < destinyValues.Length; i++, j++)
        for (long i = startIndex, j = 0; j < length && i < destinyValues.Length; i++, j++)
        //old for (long i = startIndex, j = 0; i <= length; i++, j++)
        {
            destinyValues[i] = isChar ? ' ' : fromValues[(int)j];
        }
    ...
    {
        // tratamento para numeros inteiros que tem 0 adcionados a esquerda como 01. quando entra no metodo perde a casa e ocorre exceção. 
        if (from.ToString().All(char.IsDigit) && length != from.ToString().Length)
            from = from.ToString().PadLeft((int)length, '0');
    ...
    public static void Multiply(int value, VarBasis result)
    {
        if (result is IntBasis intResult)
        {
            intResult.Value = intResult.Value * value;
        }
        else if (result is DoubleBasis doubleResult)
        {
            doubleResult.Value = doubleResult.Value * (double)value;
        }
        else
        {
    ...
    {
        return !string.IsNullOrEmpty(input) && input.All(c => (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z'));
    }
}
