...
DTH-PROCESSAMENTO.
      ...
      *--------------*                                                          
       0000-PRINCIPAL.                                                          
      *--------------*                                                       
               EXEC SQL DECLARE CUR1 WITH RETURN WITH HOLD FOR SELECT NUM_PROPOSTA, NUM_CERTIFICADO, NUM_PARCELA, NUM_APOLICE, NUM_ENDOSSO, SEQ_CONTROLE_SIGCB, NUM_OCORR_MOVTO, NUM_IDLG, COD_PRODUTO, DTA_MOVIMENTO, DTA_VENCIMENTO, VLR_PREMIO_TOTAL, VLR_IOF, QTD_PARCELA, QTD_DIAS_CUSTODIA, COD_CLIENTE, COD_CEDENTE_SAP, NUM_BOLETO_INTERNO, NUM_NOSSO_NUMERO_SAP, COD_LINHA_DIGITAVEL, NUM_TITULO, IDE_SISTEMA, COD_USUARIO, COD_SITUACAO, DTH_PROCESSAMENTO FROM SEGUROS.GE_CONTROLE_EMISSAO_SIGCB WHERE NUM_APOLICE = :NUM_APOLICE AND NUM_ENDOSSO = :NUM_ENDOSSO AND SEQ_CONTROLE_SIGCB = (SELECT MAX(T2.SEQ_CONTROLE_SIGCB) FROM SEGUROS.GE_CONTROLE_EMISSAO_SIGCB T2 WHERE T2.NUM_APOLICE = T1.NUM_APOLICE AND T2.NUM_PARCELA = T1.NUM_PARCELA AND T2.NUM_ENDOSSO = T1.NUM_ENDOSSO) ORDER BY NUM_APOLICE, NUM_ENDOSSO, NUM_PARCELA END-EXEC                                                              
      *    
           OPEN CUR1.	  
           STOP RUN.                                                            
