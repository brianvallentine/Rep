      ...
      *>   77  LK-TIP-CALC                        PIC X(254).
           ...
           77  char(DTMOVTO,local)                PIC X(254).
           ...
           77  char(DTVENCTO,local)               PIC X(254).
           ...
           77  RNUDOC                             PIC X(254).
           77  char(DTQITBCO,local)               PIC X(254).
...
char(DTQITBCO,local)
    COD-EMPRESA.
      ...
      *--------------*                                                          
       0000-PRINCIPAL.                                                          
      *--------------*                                                       
               EXEC SQL DECLARE CUR1 WITH RETURN WITH HOLD FOR SELECT NUM_APOLICE, NRENDOS, NRPARCEL, DACPARC, char(DTMOVTO,local), OPERACAO, HORAOPER, OCORHIST, PRM_TARIFARIO, VAL_DESCONTO, VLPRMLIQ, VLADIFRA, VLCUSEMI, VLIOCC, VLPRMTOT, VLPREMIO, char(DTVENCTO,local), BCOCOBR, AGECOBR, NRAVISO, NRENDOCA, SITCONTB, COD_USUARIO, RNUDOC, char(DTQITBCO,local), COD_EMPRESA FROM seguros.v1histoparc WHERE num_apolice = :NUM_APOLICE AND nrendos = :NRENDOS AND nrparcel = :NRPARCEL AND ocorhist = :OCORHIST END-EXEC                                                              
      *    
           OPEN CUR1.	  
           STOP RUN.                                                            
