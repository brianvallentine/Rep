...
{
    public class MoveCommand : ICobolCommand
    {
        ...
        public CobolCommandResponse Execute(string line)
        {
            ...
            if (!Regex.IsMatch(line, @"^MOVE\s+")) return response;

            var ret = line;
            var allCommands = ConvertHelper.FindCommands<IMoveCommand>(Assembly.GetExecutingAssembly()).OrderBy(x => x.Order);
            ...
            {
                var ax = new Exception($"ERRO - MOVE - {ex.Message}");
