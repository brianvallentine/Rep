...
public class FileBasis : StructBasis<string>
{
    ...
    bool HasWritedalready { get; set; } = false;
    ...
    public void Read(Action atEnd = null, Action notAtEnd = null)
    {
        ...
        try
        {
            ...
            {
                if (!AllLines.Any())
                    AllLines = File.ReadLines(FilePath, CodePagesEncodingProvider.Instance.GetEncoding(1252)).ToList();
    ...
    public void Write(string value = null)
    {
        ...
        try
        {
            var toWrite = value;
            if (HasWritedalready)
                toWrite = Environment.NewLine + toWrite;

            if (!isMemFile)
                using (var writer = new StreamWriter(FilePath, append: true, CodePagesEncodingProvider.Instance.GetEncoding(1252)))
