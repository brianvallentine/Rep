...
public class VarBasis
{
    ...
    */
    ...
    public static bool operator ==(VarBasis basis, VarBasis comparacao)
    {
        ...
        if (basis.IsHighValues != comparacao.IsHighValues) return false;
    ...
    public static bool operator >(VarBasis basis, VarBasis compare)
    {
        ...
        //comparar Double ou int
        var isBasisDouble = double.TryParse(basisData, out var pDoubleBasis);
        var isCompareDouble = double.TryParse(compareData, out var pDoubleCompare);

        if (isBasisDouble && isCompareDouble)
            return pDoubleBasis > pDoubleCompare;
    ...
    public static bool operator <(VarBasis basis, int compare)
    {
        ...
        //comparar Double ou int
        var isBasisDouble = double.TryParse(basisData, out var pDoubleBasis);
        var isCompareDouble = double.TryParse(compareData.ToString(), out var pDoubleCompare);
    ...
    public static bool operator <=(VarBasis basis, string compare)
    {
        //vamos testar se é comparação de strings provaveis datas
        var basisData = basis.GetMoveValues();
        var compareData = compare;
    ...
    private string GetSecureValue(string value, Type type, int length, int precision, List<SelectorItemBasis> selectorItens = null)
    {
        ...
        )
        {
            ...
            var isNumber = long.TryParse(localValue, out var isNP) || double.TryParse(localValue, out var isNPggt);
        ...
        if (isPicture)
        {
            ...
            if (currentType == typeof(DoubleBasis))
            {
                ...
                else
                {
                    ...
                    value = value.Insert(value.Length - precision, ",").Replace(".", "");
                }
